...
if (member.role === 'parent') {
  ...
  if (surveyFullyCompleted) {
    ...
    console.log(`Parent ${member.name} survey marked as completed, setting to step 3`);
    ...
    console.log(`Parent ${member.name} has enough habits, setting to step 2`);
  } else {
    memberData.step = 1; // Still doing habits
    console.log(`Parent ${member.name} still on habits, setting to step 1`);
  }
}
    ...
    <div className="bg-white rounded-lg shadow-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
      <EnhancedEventManager
        initialEvent={existingDueDateEvent ? {
          ...
          // Ensure attendees array is clean and valid
          attendees: Array.isArray(existingDueDateEvent.attendees) 
                     ? existingDueDateEvent.attendees.filter(id => id && id !== '-')
                     : [],
          ...
          // Initialize with empty attendees array
          attendees: [],
          cycleNumber: currentWeek
        }}
        onSave={async (event) => {
          try {
            ...
            console.log("Saving event with date:", newDate, "Event type:", event.eventType, "Category:", event.category, "Full event:", event);
            ...
            const eventDetails = {
              ...
              // Use the event's category and eventType if provided
              category: event.category || 'cycle-due-date',
              eventType: event.eventType || 'cycle-due-date',
              // Pass attendees as is
