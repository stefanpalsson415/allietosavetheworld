...
// Import new clean components that don't have hook issues
...
const KidsInterestsTab = ({ externalActiveChild }) => {
  // Context hooks
  const { 
    selectedUser, 
    familyMembers,
  ...
  const [showPersonalizedSurvey, setShowPersonalizedSurvey] = useState(false);
  ...
  // Effect to sync with external active child if provided
  useEffect(() => {
    if (externalActiveChild) {
      setActiveChild(externalActiveChild);
    }
  }, [externalActiveChild]);

  // Effect to restore selected child from localStorage if no external child provided
  useEffect(() => {
    if (familyMembers.length > 0 && !activeChild && !externalActiveChild) {
  ...
  }, [familyMembers, activeChild, externalActiveChild]);
  ...
  // Start a new personalized survey
  const startPersonalizedSurvey = () => {
    if (!familyId || !activeChild) {
      setError("Please select a child first.");
      return;
    }
    
    setShowPersonalizedSurvey(true);
  };

  // Handle personalized survey completion
  const handlePersonalizedSurveyComplete = async () => {
    try {
      // Reload interests to get updates from personalized survey
      setLoading(true);
      ...
      console.error("Error updating interests after personalized survey:", error);
      setError("There was an error updating interests. Please refresh the page.");
      setLoading(false);
    }
  };

  // Start a new simple survey (legacy)
  ...
  return (
    <div className="relative">
            ...
            <div className="relative">
              ...
              {surveyDropdownOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-lg z-50">
                  <div className="py-1">
                    <button
                      className="w-full text-left px-4 py-2 text-gray-700 hover:bg-purple-50 hover:text-purple-700 flex items-center"
                      onClick={() => {
                        setSurveyDropdownOpen(false);
                        startPersonalizedSurvey();
                      }}
                    >
                    ...
                    <button
                      className="w-full text-left px-4 py-2 text-gray-700 hover:bg-blue-50 hover:text-blue-700 flex items-center"
                      onClick={() => {
                        setSurveyDropdownOpen(false);
                        startNewSurvey();
                      }}
      ...
      {/* Child Selection - Only shown when not controlled by parent component */}
      {!externalActiveChild && (
          ...
          <div className="bg-purple-50 rounded-lg shadow-sm p-4 border border-purple-100">
            ...
            <div className="mt-3 text-center">
              <span className="text-sm text-purple-800">
                Use the "Kid Survey" button in the header to run surveys
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Survey Modal - Now using the clean component */}
      ...
      {/* Removed floating survey button to reduce UI clutter */}
      
      {/* Personalized Survey Modal - Using clean component */}
      {showPersonalizedSurvey && (
        <CleanPersonalizedKidSurvey 
          onClose={() => {
            setShowPersonalizedSurvey(false);
            handlePersonalizedSurveyComplete();
          }}
