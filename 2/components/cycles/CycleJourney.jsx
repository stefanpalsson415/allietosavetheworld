      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-8">
        {steps.map((step) => {
          ...
          if (cycleType === 'family' && 
              ...
              step.number === 1) {
            // Now children participate as helpers
            return (
              <motion.div 
                key={`action-${step.number}`}
                className="rounded-lg p-5 relative overflow-hidden bg-white border-2 border-indigo-200 shadow-md"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.1 * step.number }}
              >
                <div className="flex flex-col h-full">
                  <div className={`rounded-full w-12 h-12 flex items-center justify-center mb-3 bg-gradient-to-r ${step.color} text-white mx-auto`}>
                    {step.icon}
                  </div>
                  ...
                  <p className="text-sm text-gray-600 mb-4 text-center">
                    Be a helper for your parents' new habits! Help remind and celebrate with them.
                  </p>
                  <div className="bg-blue-50 p-3 rounded-lg text-sm text-blue-800 mb-4">
                    <p>In Allie Chat, you'll get special messages to help your parents build better habits.</p>
                  </div>
                  <button
                    onClick={() => onStartStep(step.action, step.number)}
                    className={`px-4 py-2 rounded-lg flex items-center justify-center bg-gradient-to-r ${step.color} text-white shadow-md hover:shadow-lg`}
                  >
