...
import { 
  ...
  Umbrella, List, Coffee, Clipboard, Paperclip
} from 'lucide-react';
...
const EnhancedEventManager = ({ 
  ...
  const handleSave = async () => {
    try {
      ...
      // Prepare event with explicit date formats and time preservation
      const eventToSave = {
        ...event,
        title: event.title || '',
        ...
        // IMPORTANT: Set all date formats consistently
        ...
        dateEndObj: endDateObj,
        
        // Set start/end in Google Calendar format
        start: {
          dateTime: dateObj.toISOString(),
          timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone
        },
  ...
  return (
    <div className={`bg-white rounded-lg shadow-md ${isCompact ? 'p-3' : 'p-4'} w-full font-roboto max-h-[90vh] overflow-y-auto`} data-testid="event-form">
