...
const handleForceRefresh = async () => {
  ...
  try {
    ...
    // Log events after refresh - only sample first few to prevent huge logs
    const eventsToLog = events.slice(0, 5).map(e => ({
      title: e.title,
      date: e.dateObj instanceof Date ? e.dateObj.toDateString() : 'Invalid Date',
      source: e.source
    }));
    console.log(`🔴 Events after refresh (showing ${Math.min(5, events.length)} of ${events.length}):`, eventsToLog);
  } finally {
    ...
    <div className="bg-white rounded-lg shadow-lg w-full max-h-[85vh] overflow-y-auto">
      ...
      {showSuccess && (
        <div className="absolute inset-0 flex items-center justify-center z-50 bg-black bg-opacity-20">
          <div className="bg-white rounded-lg p-6 shadow-lg animate-bounce w-full">
