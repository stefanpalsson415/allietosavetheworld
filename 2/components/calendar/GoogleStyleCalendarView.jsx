...
const GoogleStyleCalendarView = ({
  ...
  const getEventsForDate = (date) => {
    ...
    console.log(`📅 Filtering events for date: ${targetDate.toDateString()}, total events: ${events.length}`);
  ...
  // IMPORTANT: Listen for force calendar refresh events from the event manager
  useEffect(() => {
    // This will ensure the calendar view responds to updates from other components
    const handleForceRefresh = (e) => {
      console.log("📅 GoogleStyleCalendarView received force-calendar-refresh event:", e.detail);
      
      // Generate a new Date object with the same values to force React to detect a change
      setCurrentDate(prevDate => {
        const newDate = new Date(prevDate);
        // If we have an eventId in the detail and it's the selected event, update the view
        if (e.detail?.eventId && selectedEvent?.firestoreId === e.detail.eventId) {
          console.log("📅 This refresh affects the currently selected event, forcing UI update");
          if (e.detail?.eventData) {
            // If we have event data, update the selected event
            setSelectedEvent(e.detail.eventData);
          }
        }
        return newDate;
      });
    ...
    const handleEventUpdated = (e) => {
      console.log("📅 GoogleStyleCalendarView received calendar-event-updated event:", e.detail);
      
      // If we have a specific event ID that was updated
      if (e.detail?.eventId) {
        // If it's the currently selected event, update it with new data
        if (selectedEvent && (selectedEvent.firestoreId === e.detail.eventId || selectedEvent.id === e.detail.eventId)) {
          console.log("📅 Updating selected event with new data");
          
          // If event data is provided in the event, use it directly
          if (e.detail.eventData) {
            setSelectedEvent(e.detail.eventData);
          }
        }
  ...
  const renderMonthView = () => {
    ...
    return (
      <div className="month-view">
        {/* Days of week header */}
        <div className="grid grid-cols-7 text-center font-medium text-xs text-gray-500 border-b">
          {daysOfWeek.map(day => (
              ...
              >
                <div className="flex justify-between items-center mb-1">
                  <span className={`text-xs font-medium ${
                    isToday(date) ? 'bg-blue-500 text-white rounded-full w-5 h-5 flex items-center justify-center' : ''
                  }`}>
                    {date.getDate()}
                  </span>
                </div>
                ...
                <div className="overflow-hidden space-y-1">
                  {sortEventsByStartTime(dateEvents).slice(0, maxDisplayEvents).map((event, idx) => (
                    <div
                      ...
                      className={`calendar-event text-xs truncate px-1 rounded cursor-pointer ${getEventColor(event)}`}
                    >
        ...
        <div className="grid grid-cols-8 border-b sticky top-0 bg-white z-10 shadow-sm">
          ...
          {daysOfWeek.map((date, index) => {
            ...
            return (
              <div 
                key={index}
                className={`h-14 border-r flex flex-col items-center justify-center ${
              ...
              >
                ...
                <div className={`text-sm font-semibold mt-1 ${
                  isDateToday ? 'bg-blue-500 text-white rounded-full w-7 h-7 flex items-center justify-center' : ''
                }`}>
                  {dayNumber}
                </div>
              </div>
                  ...
                  <div className="relative">
                    ...
                    {dateEvents.map((event, idx) => {
                      ...
                      return (
                        <div
                          key={`${event.id || event.firestoreId || idx}`}
                          className={`calendar-event absolute left-0 right-0 mx-1 p-1 rounded cursor-pointer overflow-hidden ${getEventColor(event)}`}
    ...
    return (
      <div className="day-view h-full overflow-hidden flex flex-col">
        ...
        <div className="grid grid-cols-2 border-b sticky top-0 bg-white z-10 shadow-sm">
          ...
          {/* Day header */}
          <div className={`h-14 border-b flex flex-col items-center justify-center ${
            isToday(currentDate) ? 'bg-blue-50' : ''
          }`}>
            <div className="text-md">{dayName}</div>
            <div className={`text-lg font-semibold mt-1 ${
              isToday(currentDate) ? 'bg-blue-500 text-white rounded-full w-8 h-8 flex items-center justify-center' : ''
            }`}>
              {dayNumber}
            </div>
          </div>
