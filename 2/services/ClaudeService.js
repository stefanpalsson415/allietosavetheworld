...
// Method for interactive family meeting chat responses
async getChatResponse(userMessage, systemPrompt, contextInfo) {
  try {
    console.log('Getting chat response from Claude with context:', { userMessage, hasSystemPrompt: !!systemPrompt });
    
    // Default system prompt for family meeting guidance if none provided
    const defaultSystemPrompt = `You are Allie, an AI family assistant helping guide a family through their weekly meeting.
    
Be warm, friendly, and encouraging. Respond to the family's input with helpful guidance and keep the meeting on track.
    
After your response, suggest 2-3 possible responses for the family, formatted like this:
[SUGGESTIONS]: We've been doing great with habit tracking, We need more help with mornings, Let's talk about what the kids said`;
    ...
    if (contextInfo) {
      try {
        parsedContext = typeof contextInfo === 'string' ? JSON.parse(contextInfo) : contextInfo;
      } catch (e) {
        console.error('Error parsing context info:', e);
      }
    }
    
    // Extract family context if available
    const {
      familyId,
      familyMembers = [],
      currentWeek = 1,
    ...
    // Construct human message with context for Claude
    let contextualizedMessage = userMessage;
    if (Object.keys(parsedContext).length > 0) {
      // Add family meeting context if available
      if (stage) {
        contextualizedMessage = `[FAMILY MEETING CONTEXT]
Current stage: ${stage}
...
${userMessage}`;
    ...
    // Try to use the local proxy first
    try {
      ...
      console.log('Received response from Claude for chat');
      return claudeResponse;
    } catch (proxyError) {
      console.warn('Local proxy failed, trying fallback URL:', proxyError);
      
      // Try the fallback URL
      const fallbackResponse = await fetch(this.fallbackProxyUrl, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      const fallbackClaudeResponse = fallbackData.content?.[0]?.text || '';
      
      console.log('Received response from Claude fallback for chat');
      return fallbackClaudeResponse;
    }
  } catch (error) {
    console.error('Error getting chat response from Claude:', error);
    
    // Provide a fallback response for family meetings
    return `I'm having trouble connecting to my language processing system right now. Let's continue our family meeting. What would you like to discuss next?

[SUGGESTIONS]: Let's continue with what went well, Let's talk about challenges, Let's wrap up the meeting`;
  }
}
