...
standardizeEvent(eventData) {
  ...
  let startDate = null;
  try {
    // Debug logging only if we're in development mode to reduce console spam
    ...
    if (eventData.dateObj instanceof Date && !isNaN(eventData.dateObj.getTime())) {
      ...
      if (isDevelopment) console.log("🔴 Using dateObj for start date:", startDate.toISOString());
    } else if (eventData.start?.dateTime) {
      startDate = new Date(eventData.start.dateTime);
      if (isDevelopment) console.log("🔴 Using start.dateTime for start date:", startDate.toISOString());
    } else if (eventData.dateTime) {
      startDate = new Date(eventData.dateTime);
      if (isDevelopment) console.log("🔴 Using dateTime for start date:", startDate.toISOString());
    } else if (eventData.date) {
      startDate = new Date(eventData.date);
      if (isDevelopment) console.log("🔴 Using date for start date:", startDate.toISOString());
    } else {
      startDate = new Date();
      if (isDevelopment) console.log("🔴 No date found, using current time for start date");
    }
  } catch (error) {
