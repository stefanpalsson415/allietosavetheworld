...
export function SurveyProvider({ children }) {
  // DEPRECATED: Static questions - keeping for backwards compatibility only
  // New surveys will use DynamicSurveyGenerator
  ...
  const generateWeeklyQuestions = async (weekNumber, isChild = false, familyData = null, previousResponses = {}, taskCompletionData = [], childId = null) => {
    ...
    const analyzeImbalancesByCategory = async () => {
      ...
      if (familyData?.familyId) {
        try {
          // Get ELO-based category imbalances
          const eloImbalances = await ELORatingService.getCategoryImbalances(familyData.familyId);
          
          // Transform ELO data to match the expected format
          const imbalanceData = Object.entries(eloImbalances).map(([category, data]) => {
            const total = data.mamaRating + data.papaRating;
            const mamaPercent = (data.mamaRating / total) * 100;
            const papaPercent = (data.papaRating / total) * 100;
            
            return {
              category,
              mama: Math.round(mamaPercent * data.matchCount / 100), // Approximate count
              papa: Math.round(papaPercent * data.matchCount / 100),
              total: data.matchCount,
              imbalance: Math.abs(mamaPercent - papaPercent),
              dominantParent: data.leader,
              mamaPercent,
              papaPercent,
              confidence: data.confidence,
              eloScore: data.score // Keep the raw ELO score difference
            };
          });
          
          // Sort by ELO score (which represents true imbalance)
          return imbalanceData.sort((a, b) => b.eloScore - a.eloScore);
        } catch (error) {
          console.error('Error fetching ELO imbalances, falling back to survey responses:', error);
          // Fall back to original implementation
        }
      }
      
      // Fallback: Use child-specific responses if available
    ...
    const identifyBalancedCategories = async () => {
      const imbalances = await analyzeImbalancesByCategory();
      // Consider categories with less than 20% imbalance as "balanced"
      return imbalances.filter(data => data.imbalance < 20);
    };
    
    // Helper to get uncovered tasks from ELO data
    const getUncoveredTasks = async () => {
      if (familyData?.familyId) {
        try {
          const uncoveredData = await ELORatingService.getUncoveredTasks(familyData.familyId);
          return uncoveredData;
        } catch (error) {
          console.error('Error fetching uncovered tasks:', error);
        }
      }
      return { total: 0, topUncoveredTasks: [], byCategory: {} };
    };
    ...
    // Helper to get questions from a specific category, prioritizing uncovered tasks and by weight
    const getQuestionsFromCategory = (category, count, excludeIds = [], uncoveredTasks = {}) => {
      ...
      // Separate uncovered task questions and regular questions
      const uncoveredQuestions = [];
      const regularQuestions = [];
      
      categoryQuestions.forEach(question => {
        const taskType = question.taskType || question.text;
        if (uncoveredTasks.byTask && uncoveredTasks.byTask[taskType]) {
          uncoveredQuestions.push({
            ...question,
            uncoveredCount: uncoveredTasks.byTask[taskType],
            priorityBoost: 100 // High priority for uncovered tasks
          });
        } else {
      ...
      // Sort uncovered questions by how often they're uncovered
      uncoveredQuestions.sort((a, b) => b.uncoveredCount - a.uncoveredCount);
      
      // Sort regular questions by total weight (highest impact first)
      regularQuestions.sort((a, b) => 
        parseFloat(b.totalWeight) - parseFloat(a.totalWeight)
      );
      
      // Combine: uncovered first, then regular
      const combinedQuestions = [...uncoveredQuestions, ...regularQuestions];
    ...
    const finalQuestions = selectedQuestions.map((question, index) => {
      ...
      return {
        ...question,
        id: weeklyQuestionId, // Override the original ID with the weekly/cycle ID
        originalId: question.id, // Keep the original ID for reference
        weeklyExplanation
      };
    });
  ...
  // Initial questions - memoized to prevent recreation on every render
  const [fullQuestionSet] = useState(() => generateFullQuestionSet());
  ...
  // State to track when responses have been updated
  const [lastResponseUpdate, setLastResponseUpdate] = useState(Date.now());

  // Track current personalized questions for balance forecast
  const [currentPersonalizedQuestions, setCurrentPersonalizedQuestions] = useState(null);

  // Store dynamic questions separately to ensure they're available for ELO updates
  const [dynamicQuestionMap, setDynamicQuestionMap] = useState({});

  // Add or update a survey response
  // Add debounced save reference
  ...
  const updateSurveyResponse = useCallback(async (questionId, answer, questionData = null) => {
    ...
    // Update the timestamp to trigger re-renders in components watching for changes
    // Add a small delay to ensure Firestore write is complete before triggering UI updates
    setTimeout(() => {
      setLastResponseUpdate(Date.now());
    }, 100);
    
    // Update ELO ratings if we have family context
    if (familyDataState?.familyId) {
      try {
        // Find the question - check dynamic questions first, then static
        let question = questionData || dynamicQuestionMap[questionId] || fullQuestionSet.find(q => q.id === questionId);
        ...
        if (question && question.category) {
          await ELORatingService.updateRatingsForResponse(
            ...
            answer,
            {
              totalWeight: question.totalWeight || calculateTaskWeight(question, familyDataState?.priorities || {}).toFixed(2),
              text: question.text,
              taskType: question.taskType || question.text, // Use taskType if available, otherwise use text
              isDynamic: !!questionData || !!dynamicQuestionMap[questionId] // Track if this was a dynamic question
            }
          );
        } else {
          console.warn(`Question ${questionId} not found for ELO update`);
        }
      } catch (error) {
        console.error('Error updating ELO ratings:', error);
        // Don't fail the survey response if ELO update fails
      }
    }
    
    // NEW: Trigger debounced save to Firebase
    if (familyDataState?.familyId && familyDataState?.selectedUserId) {
      ...
      debouncedSaveRef.current = setTimeout(async () => {
        try {
          console.log('Debounced save: Saving survey responses to Firebase...');
          const { default: DatabaseService } = await import('../services/DatabaseService');
          
          // Get current responses including this new one
          ...
          console.log(`Debounced save: Successfully saved ${Object.keys(updatedResponses).length} responses`);
        } catch (error) {
          console.error('Debounced save failed:', error);
          // Don't fail the survey if save fails - auto-save will catch it
        }
      }, 3000); // Save after 3 seconds of no activity
    }
  }, [completedQuestions, familyDataState, fullQuestionSet, dynamicQuestionMap, currentSurveyResponsesState]);
  ...
  const setCurrentSurveyResponses = useCallback((responses) => {
  ...
  const setFamilyData = useCallback((data) => {
    setFamilyDataState(data);
  }, []);
  
  // Get personalized initial survey questions
  const getPersonalizedInitialQuestions = useCallback(() => {
    return selectPersonalizedInitialQuestions(fullQuestionSet, familyDataState, 72);
  }, [fullQuestionSet, familyDataState]);
  
  // Update weight attributes for a question
  const updateQuestionWeight = useCallback((questionId, attributeName, newValue) => {
  ...
  const generateWeeklyQuestionsWithCache = useCallback((weekNumber, isChild = false, familyData = null, previousResponses = {}, taskCompletionData = [], childId = null) => {
    // Create a cache key
  ...
  const getInvisibleWorkQuestions = useCallback(() => {
    return fullQuestionSet.filter(q => 
  ...
  const getEmotionalLaborQuestions = useCallback(() => {
    return fullQuestionSet.filter(q => 
  ...
  const getFilteredQuestionsForChild = useCallback(async (childId, familyId, baseQuestions) => {
    try {
  ...
  const getFilteredQuestionsForAdult = useCallback(async (familyId, baseQuestions, userId) => {
    try {
      // Get questions to exclude based on previous feedback for this specific user
      // Pass the userId as the memberId parameter to get user-specific exclusions
      const questionIdsToExclude = await QuestionFeedbackService.getQuestionsToExclude(familyId, userId);
      
      // Filter out questions that were marked as not applicable by this specific user
      return baseQuestions.filter(question => !questionIdsToExclude.includes(question.id));
    } catch (error) {
  ...
  // Wrap selectPersonalizedInitialQuestions in useCallback
  const memoizedSelectPersonalizedInitialQuestions = useCallback((questionSet, familyData, targetCount = 72) => {
    return selectPersonalizedInitialQuestions(questionSet, familyData, targetCount);
  }, []);

  // Memoize filtered question sets to prevent recreating arrays on every render
  const filteredFullQuestionSet = useMemo(() => 
    fullQuestionSet.filter(q => q.category !== "Relationship Health"),
    [fullQuestionSet]
  );
  
  const relationshipQuestionsSet = useMemo(() => 
    fullQuestionSet.filter(q => q.category === "Relationship Health"),
    [fullQuestionSet]
  );

  // Generate dynamic personalized questions using AI
  const generateDynamicQuestions = useCallback(async (familyId, memberId, targetCount = 72) => {
    try {
      ...
      // Add weight calculations to each question
      const enhancedQuestions = questions.map(q => ({
        ...q,
        totalWeight: calculateTaskWeight(q, familyDataState?.priorities || {}).toFixed(2)
      }));
      
      // Store dynamic questions in map for ELO tracking
      const newQuestionMap = {};
      enhancedQuestions.forEach(q => {
        newQuestionMap[q.id] = q;
      });
      setDynamicQuestionMap(prev => ({ ...prev, ...newQuestionMap }));
      
      return enhancedQuestions;
    } catch (error) {
      console.error('Error generating dynamic questions:', error);
      // Fallback to static questions
      return memoizedSelectPersonalizedInitialQuestions(filteredFullQuestionSet, familyDataState, targetCount);
    }
  }, [familyDataState, filteredFullQuestionSet, memoizedSelectPersonalizedInitialQuestions]);

  // Update the context value
  const value = useMemo(() => ({
    // Filter out relationship questions for regular survey
    fullQuestionSet: filteredFullQuestionSet,
    generateWeeklyQuestions: generateWeeklyQuestionsWithCache,
    generateDynamicQuestions, // NEW: AI-powered question generation
    ...
    relationshipQuestions: relationshipQuestionsSet,
    ...
    selectPersonalizedInitialQuestions: memoizedSelectPersonalizedInitialQuestions, // NEW: Also expose the direct function
    // Getter functions
    ...
    // NEW: Add lastResponseUpdate to trigger re-renders when responses change
    lastResponseUpdate,
    // Current personalized questions for balance forecast
    currentPersonalizedQuestions,
    setCurrentPersonalizedQuestions
  }), [
    filteredFullQuestionSet,
    generateWeeklyQuestionsWithCache,
    ...
    getSurveyProgress,
    setCurrentSurveyResponses,
    relationshipQuestionsSet,
    userModifiedWeights,
    updateQuestionWeight,
    setFamilyData,
    getPersonalizedInitialQuestions,
    memoizedSelectPersonalizedInitialQuestions,
    getQuestionsByCategory,
    getHighImpactQuestions,
    getRelationshipImpactQuestions,
    getInvisibleWorkQuestions,
