...
import { clearUserAvatarCache } from '../components/common/UserAvatar';
...
export function FamilyProvider({ children }) {
  ...
  // Expose context globally for debugging and profile switching
  useEffect(() => {
  ...
  }, [familyId, selectedUser, familyMembers]);

  // Load fresh family data from Firestore (background refresh)
  const loadFreshFamilyData = useCallback(async (options = {}) => {
    if (!familyId) return;
    
    // Options: { force: boolean } - force refresh even if we have data
    const { force = false } = options;
    
    // Skip if we already have recent data (unless forced)
    if (!force && familyMembers.length > 0) {
      ...
      const members = typeof familyMembers === 'object' && !Array.isArray(familyMembers)
        ...
        : familyMembers;
      const hasRecentData = (Array.isArray(members) ? members : []).some(m =>
        m.profilePicture?.includes('firebasestorage.googleapis.com') ||
        m.profilePictureUrl?.includes('firebasestorage.googleapis.com')
      );
      if (hasRecentData) {
        console.log('Family data is recent, skipping refresh');
        return;
      }
    }
    
    try {
      console.log('Loading fresh family data from Firestore...');
      const familyDoc = await getDoc(doc(db, 'families', familyId));
      
      if (familyDoc.exists()) {
        const freshData = familyDoc.data();
        console.log('Fresh family data loaded:', freshData);
        
        // Update family members with fresh data
        if (freshData.familyMembers) {
          setFamilyMembers(freshData.familyMembers);
          console.log('Updated family members with fresh profile pictures');
        }
        
        // Update other family data as needed
        if (freshData.familyPicture) setFamilyPicture(freshData.familyPicture);
        if (freshData.familyName) setFamilyName(freshData.familyName);
      }
    } catch (error) {
      console.error('Error loading fresh family data:', error);
    }
  ...
  useEffect(() => {
    const initializeFamilyData = async () => {
      if (initialFamilyData) {
        ...
        // Check if enhanced profiles need to be initialized for existing families
        if (initialFamilyData.familyId && !initialFamilyData.enhancedProfiles) {
          try {
            console.log("Initializing enhanced profiles for existing family...");
            await DatabaseService.initializeEnhancedProfilesForFamily(initialFamilyData.familyId);
          } catch (profileError) {
        ...
        // Load survey responses if not already loaded
        if (initialFamilyData.familyId && (!initialFamilyData.surveyResponses || Object.keys(initialFamilyData.surveyResponses).length === 0)) {
          try {
            console.log("Loading survey responses for family...");
            const dbResponses = await DatabaseService.loadSurveyResponses(initialFamilyData.familyId);
            if (dbResponses && dbResponses.allResponses && Object.keys(dbResponses.allResponses).length > 0) {
              console.log("Found survey responses in Firebase, updating state");
              setSurveyResponses(dbResponses.allResponses);
            }
          } catch (error) {
            console.error("Error loading survey responses:", error);
            // Non-critical error - continue without survey data
          }
        }
...
// Load survey responses when familyId is available
useEffect(() => {
  const loadSurveyResponsesIfNeeded = async () => {
    if (familyId && (!surveyResponses || Object.keys(surveyResponses).length === 0)) {
      try {
        console.log("FamilyContext: Loading survey responses for familyId:", familyId);
        const dbResponses = await DatabaseService.loadSurveyResponses(familyId);
        if (dbResponses && dbResponses.allResponses && Object.keys(dbResponses.allResponses).length > 0) {
          console.log("FamilyContext: Successfully loaded survey responses:", Object.keys(dbResponses.allResponses).length, "responses");
          setSurveyResponses(dbResponses.allResponses);
        } else {
          console.log("FamilyContext: No survey responses found in database");
        }
      } catch (error) {
        console.error("FamilyContext: Error loading survey responses:", error);
      }
    }
  };
  
  loadSurveyResponsesIfNeeded();
}, [familyId]); // Only depend on familyId to avoid infinite loops

// Restore selected user from localStorage if available
useEffect(() => {
  if (familyMembers.length > 0) {
    const storedUserId = localStorage.getItem('selectedUserId');
    // Check if there's a stored user ID AND either:
    // 1. No selected user yet, OR
    // 2. The selected user's ID doesn't match the stored ID (user has changed)
    if (storedUserId && (!selectedUser || selectedUser.id !== storedUserId)) {
      // familyMembers might be an object - convert to array first
      const members = typeof familyMembers === 'object' && !Array.isArray(familyMembers)
        ? Object.values(familyMembers)
        : familyMembers;
      const userFromStorage = (Array.isArray(members) ? members : []).find(member => member.id === storedUserId);
      if (userFromStorage) {
        console.log("Restoring selected user from localStorage:", userFromStorage.name);
        // Use selectFamilyMember to ensure the event is triggered
        selectFamilyMember(userFromStorage);
      }
    }
  }
}, [familyMembers]); // Remove selectedUser from deps to avoid infinite loop
...
updateFavicon(familyPicture || getDefaultFavicon());
  ...
  const selectFamilyMember = (member) => {
    ...
    if (member && member.id) {
      ...
      console.log(`Setting selectedUserId in localStorage to: ${member.id}`);
      
      // Update the state
      setSelectedUser(member);
      
      // Dispatch event for other parts of the app to know about the switch
      ...
      // Update global reference for services
      if (window._currentSelectedUser !== member) {
        window._currentSelectedUser = member;
      }
    } else {
      console.warn("Attempted to select invalid family member:", member);
    }
    return member;
  };
  ...
  const updateMemberProfile = async (memberId, data) => {
    try {
      ...
      const members = typeof familyMembers === 'object' && !Array.isArray(familyMembers)
        ? Object.values(familyMembers)
        : familyMembers;
      const updatedMembers = (Array.isArray(members) ? members : []).map(member =>
        member.id === memberId ? { ...member, ...data } : member
      );

      setFamilyMembers(updatedMembers);
      
      // Deep sanitize the data before saving to remove any non-serializable values
      const sanitizeForFirestore = (obj) => {
        ...
        Object.keys(obj).forEach(key => {
          ...
          if (value === undefined || typeof value === 'function') {
            return; // Skip undefined and functions
          }
      ...
      const sanitizedMembers = updatedMembers.map(member => sanitizeForFirestore(member));
      
      // Save to Firebase
      await DatabaseService.saveFamilyData({ familyMembers: sanitizedMembers }, familyId);
      
      // Clear avatar caches if profile picture was updated
      if (data.profilePicture || data.profilePictureUrl) {
...
const completeRelationshipAssessment = async (cycleNumber, responses) => {
  try {
    ...
    const members = typeof familyMembers === 'object' && !Array.isArray(familyMembers)
      ? Object.values(familyMembers)
      : familyMembers;
    const parentMembers = (Array.isArray(members) ? members : []).filter(m => m.role === 'parent');
  ...
  try {
    ...
    try {
      ...
      const members = typeof familyMembers === 'object' && !Array.isArray(familyMembers)
        ? Object.values(familyMembers)
        : familyMembers;
      const parents = (Array.isArray(members) ? members : []).filter(m => m.role === 'parent');
...
const updateRelationshipStrategy = async (strategyId, updateData) => {
  ...
  const completeInitialSurvey = async (memberId, responses, skipSave = false) => {
  try {
    console.log(`Starting initial survey completion for member ${memberId}`);
    console.log(`Skip save flag: ${skipSave}, Response count: ${Object.keys(responses).length}`);
    if (!familyId) throw new Error("No family ID available");
    ...
    try {
      ...
      // Only save responses if not already saved to avoid double-save
      if (!skipSave) {
        ...
        console.log("Skipping survey response save (already saved)");
      }
      ...
      setFamilyMembers(updatedMembers);
      // Prefix responses with member ID to keep them isolated
      const prefixedResponses = {};
      Object.entries(responses).forEach(([key, value]) => {
        prefixedResponses[`${memberId}_${key}`] = value;
      });
      setSurveyResponses({ ...surveyResponses, ...prefixedResponses });
      ...
      if (selectedUser && selectedUser.id === memberId) {
        setSelectedUser({
          ...
          completedDate: new Date().toISOString().split('T')[0],
          surveys: {
            ...selectedUser.surveys,
            initial: {
              completed: true,
              completedAt: new Date().toISOString(),
              responses: selectedUser.surveys?.initial?.responses || {},
              responseCount: Object.keys(responses).length
            }
          }
        });
...
const extractAIPreferencesFromSurvey = (responses) => {
  ...
  const saveSurveyProgress = async (memberId, responses) => {
    try {
      ...
      // Update local survey responses state with member-prefixed keys
      const prefixedResponses = {};
      Object.entries(responses).forEach(([key, value]) => {
        // Only add prefix if the key doesn't already have a member prefix
        const prefixedKey = key.includes('_') ? key : `${memberId}_${key}`;
        prefixedResponses[prefixedKey] = value;
      });
      setSurveyResponses({
        ...surveyResponses,
        ...prefixedResponses
      ...
      // Update local family members state with responseCount
      ...
      // Try to update the member's survey progress in Firebase, but don't fail if permissions issue
      try {
        await DatabaseService.updateMemberSurveyProgress(familyId, memberId, 'initial', responseCount);
      } catch (progressError) {
        console.warn("Could not update member survey progress in family doc (likely permissions issue):", progressError);
        // Continue anyway - the main survey responses are saved
      }
...
const completeWeeklyCheckIn = async (memberId, weekNum, responses) => {
  ...
  const generateTaskForArea = (taskId, assignedTo, areaData, weekNumber) => {
    ...
    const getParentName = (role) => {
      ...
      const members = typeof familyMembers === 'object' && !Array.isArray(familyMembers)
        ? Object.values(familyMembers)
        : familyMembers;
      const parent = (Array.isArray(members) ? members : []).find(m => m.role === 'parent' && m.roleType === role);
  ...
  const completeWeek = async (weekNumber) => {
    try {
      ...
      const weekData = {
        weekNumber,
        familyMembers: (Array.isArray(members) ? members : []).map(m => ({
          id: m.id,
          name: m.name,
          role: m.role,
      ...
      // familyMembers might be an object - convert to array first
      const membersArray = typeof familyMembers === 'object' && !Array.isArray(familyMembers)
        ? Object.values(familyMembers)
        : familyMembers;
      const updatedMembers = (Array.isArray(membersArray) ? membersArray : []).map(member => {
      ...
      // Prepare the data to save, making sure to avoid null values
      const saveData = {
        weekHistory: updatedHistory,
        weekStatus: updatedStatus,
        lastCompletedFullWeek: newLastCompletedWeek,
      ...
      // Create cycleProgress update with proper nesting to avoid null values
      // Check if cycleProgress already exists for this cycle
      const cycleProgressRef = await DatabaseService.getDoc(`families/${familyId}`);
      if (cycleProgressRef.exists()) {
        const data = cycleProgressRef.data();
        // Make sure the cycleProgress object exists and has the current cycle
        const existingCycleProgress = data.cycleProgress || {};
        
        // Create the nested object structure if it doesn't exist
        if (!existingCycleProgress[currentWeek]) {
          existingCycleProgress[currentWeek] = {};
        }
        
        // Make sure the meeting object exists
        if (!existingCycleProgress[currentWeek].meeting) {
          existingCycleProgress[currentWeek].meeting = {};
        }
        
        // Now we can safely set the values
        existingCycleProgress[currentWeek].meeting.completed = true;
        existingCycleProgress[currentWeek].step = 3;
        
        // Add the updated cycleProgress to our save data
        saveData.cycleProgress = existingCycleProgress;
      } else {
        // Create from scratch with proper nesting
        saveData.cycleProgress = {
          [currentWeek]: {
            meeting: {
              completed: true
            },
      ...
      await DatabaseService.saveFamilyData(saveData, familyId);
...
const loadCurrentWeekTasks = async () => {
    ...
    if (Object.keys(surveyResponses).length === 0) {
      try {
        ...
        if (dbResponses && dbResponses.allResponses && Object.keys(dbResponses.allResponses).length > 0) {
          ...
          // Use allResponses which contains the combined responses with member prefixes
          surveyResponses = dbResponses.allResponses;
          // Store the by-member data separately if needed
          if (dbResponses.responsesByMember) {
            console.log("Responses by member:", Object.keys(dbResponses.responsesByMember));
          }
        }
      } catch (e) {
  ...
  // Get ELO ratings for the family
  const getELORatings = async () => {
    try {
      if (!familyId) throw new Error("No family ID available");
      return await ELORatingService.getFamilyRatings(familyId);
    } catch (error) {
  ...
  // Get ELO-based task recommendations
  const getELORecommendations = async () => {
    try {
      if (!familyId) throw new Error("No family ID available");
      return await ELORatingService.getTaskRecommendations(familyId);
    } catch (error) {
      console.error('Error getting ELO recommendations:', error);
      return [];
    }
  };
  
  // Get category imbalances from ELO
  const getELOCategoryImbalances = async () => {
    try {
      if (!familyId) throw new Error("No family ID available");
      return await ELORatingService.getCategoryImbalances(familyId);
    } catch (error) {
      console.error('Error getting ELO imbalances:', error);
      return {};
    }
  };
  
  // Get recent match history with weight data
  const getRecentMatchHistory = async (limitCount = 10) => {
    try {
      if (!familyId) throw new Error("No family ID available");
      return await ELORatingService.getRecentMatchHistory(familyId, limitCount);
    } catch (error) {
  ...
  const getWeightStatistics = async () => {
    try {
      if (!familyId) throw new Error("No family ID available");
      return await ELORatingService.getWeightStatistics(familyId);
    } catch (error) {
  ...
  // Get total survey response count across all family members
  const getTotalSurveyResponseCount = async (roleFilter = 'all', forceRefresh = false) => {
    try {
      if (!familyId) throw new Error("No family ID available");
      
      // Use the new filtered method to get role-specific data
      const filteredData = await ELORatingService.getFilteredSurveyResponses(familyId, roleFilter, forceRefresh);
      
      // Transform to match expected format
      return {
        totalSurveyResponses: filteredData.totalCount,
        memberCount: roleFilter === 'parent' ? filteredData.parentMemberCount : 
                     roleFilter === 'child' ? filteredData.childMemberCount :
                     filteredData.parentMemberCount + filteredData.childMemberCount,
        responsesByMember: filteredData.responsesByMemberWithRoles,
        eloProcessedResponses: filteredData.totalCount,
        aggregatedTotal: filteredData.totalCount,
        parentResponseCount: filteredData.parentResponseCount,
        childResponseCount: filteredData.childResponseCount,
        memberRoleMap: filteredData.memberRoleMap
      };
    } catch (error) {
