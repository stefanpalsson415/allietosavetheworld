...
import eventStore from '../services/EventStore'; // Use main EventStore with permanent fixes
      ...
      const loadEvents = async () => {
        try {
          ...
          if (cycleNumber && familyId) {
            ...
            // Get all events for user - PASS familyId!
            console.log('useEvent hook - Loading events with:', {
              userId: currentUser.uid,
              familyId: familyId || 'not provided',
              startDate,
              endDate
            });
            eventData = await eventStore.getEventsForUser(currentUser.uid, startDate, endDate, familyId);
          }
