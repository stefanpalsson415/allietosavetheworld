...
// We've split the LTVExpansionSlide into two separate slides
...
const InvestorFunnel = () => {
    ...
    // Initialize notification with null (no notification shown)
    const [notification, setNotification] = useState(null);
    const [interactionNotifShown, setInteractionNotifShown] = useState(false);
    
    const totalSlides = 27; // Updated after splitting LTV Expansion slide into two
    
    // Track previous slide for notification control
    const [previousSlide, setPreviousSlide] = useState(1);
    
    // Helper function to determine if a slide has interactive elements
    const hasInteractiveElements = (slide) => {
      // List of slides with interactive elements (updated for our new slide order)
      const interactiveSlides = [5, 6, 7, 8, 13, 14, 15, 16, 17, 18];
      return interactiveSlides.includes(slide);
    };
    ...
    // Changes made:
    // 1. Increased spacing from title to top of browser in slide components
    // 2. Only show popup when user changes slides (not on interaction)
    // 3. Removed "Core Product: Family Management" slide
    // 4. Moved "Breaking the Cycle: Children as Agents of Change" to follow "Why Mental Load Persists Across Generations"
    useEffect(() => {
      ...
      document.title = `Investor Presentation: ${slideTitles[currentSlide] || 'Allie'}`;
    ...
    const slideTitles = {
      ...
      3: "Global Crisis Impact",
      4: "Perception Gap",
      5: "Parental Gap Data",
      6: "Generational Cycle",
      7: "Breaking the Cycle: Children as Agents of Change", // Moved "Children's Role" up to here
      8: "The Sibling Advantage", // New slide about sibling dynamics
      9: "Workload Visualization",
      10: "Time Value Analysis",
      11: "Relationship Impact",
      12: "Scientific Foundation",
      // "Core Product" slide removed
      13: "Personalized Approach",
      14: "Family Flywheel - Trust Layers",
      15: "Family Data Graph",
      16: "Lifetime Value Model",
      17: "Competitive Landscape",
      18: "Market Validation",
      19: "Technology Stack",
      20: "Team & Advisors",
      21: "Product Roadmap",
      22: "Mental Load Assessment",
    ...
    const skipSlides = []; // No slides to skip anymore

 // Updated navigation logic for reordered slides

 const nextSlide = () => {
  // Simply proceed to the next slide without any blocking
  // We'll let the user navigate freely
  
  let newSlide = currentSlide + 1;
  // Update previous slide state for notification control
  setPreviousSlide(currentSlide);
  
  // Skip any slides in the skipSlides array (if any are still defined)
...
const goToSlide = (slide) => {
  if (slide >= 1 && slide <= totalSlides && !skipSlides.includes(slide)) {
    ...
    useEffect(() => {
      ...
      // Show notifications for interactive slides
      if (currentSlide === 2 && !quizAnswers.sleepLoss) {
        setNotification("Give the question a try! You'll learn something interesting.");
        // Auto-dismiss after 5 seconds
        setTimeout(() => setNotification(null), 5000);
      }
      else if (currentSlide === 4 && (!quizAnswers.fatherPercent || !quizAnswers.motherPercent)) {
        setNotification("This quiz reveals a fascinating insight about family dynamics!");
        setTimeout(() => setNotification(null), 5000);
      }
      else if (currentSlide === 5) {
        setNotification("Explore the charts to see the parental gap data across regions!");
        setTimeout(() => setNotification(null), 5000);
      }
      else if (currentSlide === 6) {
        setNotification("Click on each section to learn why mental load persists across generations!");
        setTimeout(() => setNotification(null), 5000);
      }
      else if ((currentSlide === 13 || currentSlide === 14 || currentSlide === 15 || currentSlide === 16)) {
        setNotification("Click on the elements to explore more details!");
        setTimeout(() => setNotification(null), 5000);
      }
      else {
        // Clear any existing notification when changing to non-interactive slides
  ...
  const Notification = () => {
    ...
    return (
      <div className="fixed top-4 left-1/2 transform -translate-x-1/2 bg-purple-100 border border-purple-300 text-purple-800 px-6 py-3 rounded-lg shadow-lg z-50 flex items-center max-w-md">
        ...
        <button 
          className="ml-4 text-purple-500 hover:text-purple-700"
    ...
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-6 py-3 flex justify-between items-center z-10">
      ...
      <div className="text-sm font-medium">
        {slideTitles[currentSlide] ? 
          <span>{currentSlide}: {slideTitles[currentSlide]}</span> : 
  ...
  // Slides content
  const renderSlide = () => {
    // For our new investor slides, we use the imported components
    if (currentSlide === 5) {
      return <ParentalGapSlide />;
    } else if (currentSlide === 6) {
      return <GenerationalImbalanceSlide />;
    } else if (currentSlide === 7) {
      // Breaking the Cycle: Children as Agents of Change is now slide 7
      return (
      ...
      // The Sibling Advantage slide
      return <SiblingDynamicsSlide />;
    } else if (currentSlide === 16) {
      // Slide 16: The Onion Flywheel (Replaced LTVExpansionSlide)
      return (
        <div className="min-h-[80vh] flex flex-col justify-center px-8 pt-0">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-light mb-2">The Onion Flywheel</h2>
            ...
            <div className="flex flex-col items-center bg-white rounded-lg shadow p-6 mb-6">
              <div className="mb-4 w-full">
                <h3 className="text-xl font-medium mb-3 text-center">Four Trust Layers: The Onion Flywheel</h3>

                {/* New cleaner layout with a completely redesigned onion - improved spacing */}
                <div className="relative mx-auto" style={{width: '500px', height: '500px'}}>
                  ...
                  <div className="absolute rounded-full bg-blue-50 border-4 border-blue-300"
                       style={{top: '35%', left: '35%', right: '35%', bottom: '35%'}}>
                    {/* Number badge */}
                  ...
                  <div className="absolute rounded-full bg-purple-50 border-4 border-purple-300"
                       style={{top: '58%', left: '58%', right: '58%', bottom: '58%'}}>
                    {/* Number badge - side positioned to avoid overlap with center */}
                  ...
                  <div className="absolute rounded-full bg-purple-600 flex items-center justify-center text-white shadow-lg"
                       style={{top: '72%', left: '72%', right: '72%', bottom: '72%'}}>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <div className="mb-5 bg-indigo-50 p-4 rounded-lg shadow-sm">
                <div className="font-bold mb-2 text-indigo-800 text-xl">The Flywheel Effect:</div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-gradient-to-br from-purple-50 to-indigo-50 p-4 rounded-lg">
                  <h3 className="text-lg font-medium text-indigo-800 mb-2">Trust Drives Engagement</h3>
                  <p className="text-sm">
                    As families engage with more trust layers, their monthly active usage increases exponentially:
                  </p>
                  <div className="mt-3 grid grid-cols-2 gap-2 text-center">
                    <div className="bg-white p-2 rounded shadow-sm">
                      <div className="text-xl font-bold text-purple-700">197%</div>
                      <div className="text-xs">Usage increase with each additional trust layer</div>
                    </div>
                    <div className="bg-white p-2 rounded shadow-sm">
                      <div className="text-xl font-bold text-indigo-700">85%</div>
                      <div className="text-xs">Annual retention with 3+ layers</div>
                    </div>
                  </div>
                </div>

                <div className="bg-gradient-to-br from-blue-50 to-green-50 p-4 rounded-lg">
                  <h3 className="text-lg font-medium text-blue-800 mb-2">Data Amplifies Value</h3>
                  <p className="text-sm">
                    Our privacy-first data trust model creates a compounding advantage:
                  </p>
                  <div className="mt-3 grid grid-cols-2 gap-2 text-center">
                    <div className="bg-white p-2 rounded shadow-sm">
                      <div className="text-xl font-bold text-blue-700">42%</div>
                      <div className="text-xs">Higher conversion rate on layered recommendations</div>
                    </div>
                    <div className="bg-white p-2 rounded shadow-sm">
                      <div className="text-xl font-bold text-green-700">3.4×</div>
                      <div className="text-xs">LTV multiple for full-stack families</div>
                    </div>
                  </div>
                </div>
              </div>
      ...
      // Slide 17: Family Data & LTV Capture (New slide split from LTVExpansionSlide)
      return (
        <div className="min-h-[80vh] flex flex-col justify-center px-8 pt-0">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-light mb-2">Family Data & LTV Capture</h2>
            <h3 className="text-xl font-light text-gray-600 mb-8">Monetizing trust across major family spending categories</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="flex items-start bg-gradient-to-r from-purple-50 to-white p-3 rounded">
                      <div className="bg-purple-100 p-2 rounded-full mr-2 flex-shrink-0">
                        <Calendar className="text-purple-700 h-4 w-4" />
                      </div>
                      <div>
                    ...
                    <div className="flex items-start bg-gradient-to-r from-blue-50 to-white p-3 rounded">
                      <div className="bg-blue-100 p-2 rounded-full mr-2 flex-shrink-0">
                        <Users className="text-blue-700 h-4 w-4" />
                      </div>
                      <div>
                    ...
                    <div className="flex items-start bg-gradient-to-r from-green-50 to-white p-3 rounded">
                      <div className="bg-green-100 p-2 rounded-full mr-2 flex-shrink-0">
                        <Heart className="text-green-700 h-4 w-4" />
                      </div>
                      <div>
                    ...
                    <div className="flex items-start bg-gradient-to-r from-amber-50 to-white p-3 rounded">
                      <div className="bg-amber-100 p-2 rounded-full mr-2 flex-shrink-0">
                        <Users className="text-amber-700 h-4 w-4" />
                      </div>
                      <div>
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  <div className="mt-4 bg-indigo-100 p-4 rounded-lg text-sm mb-6">
                    <div className="font-bold text-indigo-800 mb-1 text-lg">Data Trust Flywheel:</div>
                    <p>
                      The better we serve families with our data, the more data they share.
                      Each layer of trust unlocks new monetization opportunities, creating
                      a virtuous cycle of value for both families and our platform.
                    </p>
                  </div>

                  <div className="mt-5 bg-gradient-to-br from-indigo-600 to-purple-700 p-4 rounded-lg text-white">
                    <h4 className="font-medium mb-3 text-xl">The Balanced Family Growth Flywheel</h4>
                    <p>
                    ...
                    <div className="mt-4 pt-4 border-t border-white/30 flex items-center justify-between text-lg">
...
case 4:
  return (
    <div className="min-h-[80vh] flex flex-col justify-center px-8">
      ...
      // Slide 7: The Mental Load Crisis (Interactive)
      case 7:
        return (
      <div className="min-h-[80vh] flex flex-col justify-center px-8">
        <div className="max-w-4xl mx-auto">
      ...
      // Slide 11: The Solution Introduction
      case 11:
        return (
          <div className="min-h-[80vh] flex flex-col justify-center px-8">
            <div className="max-w-4xl mx-auto">
...
<div className="bg-white p-6 rounded-lg shadow-sm">
  ...
  <ul className="space-y-2 text-sm">
    ...
    <li className="flex items-start">
      ...
      // Slide 8: Scientific Foundation (Interactive)
      case 8:
        return (
          <div className="min-h-[80vh] flex flex-col justify-center px-8">
            <div className="max-w-4xl mx-auto">
  ...
  <div className="flex justify-between mt-1 px-4">
    ...
    <div className="text-xs text-gray-600">Negative</div>
      ...
      // Slide 10: Family Memory Journey
      case 10:
        return (
          <div className="min-h-[80vh] flex flex-col justify-center px-8">
            <div className="max-w-4xl mx-auto">
      ...
      // Slide 12: Relationship Impact (Core Product slide removed)
      case 12:
        return (
          <div className="min-h-[85vh] flex flex-col justify-center px-8 pt-16">
            <div className="max-w-4xl mx-auto">
              ...
              <div className="grid md:grid-cols-2 gap-8">
                ...
                <div>
                  ...
                  <div className="bg-gray-50 p-6 rounded-lg mb-2">
                    ...
                    <div className="h-80">
                      <ResponsiveContainer width="100%" height="100%">
                        <RadarChart outerRadius="60%" data={radarData} margin={{ top: 20, right: 30, bottom: 20, left: 30 }}>
                          <PolarGrid />
...
case 21:
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* In-app notification */}
      {/* Render notification using our function */}
      {renderNotification()}
