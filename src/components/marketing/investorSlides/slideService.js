/**
 * Investor Deck Slide Service
 *
 * This service handles the loading, saving, and management of slides
 * with proper version control. It uses the configuration from slideConfig.js
 * and provides a safe way to modify the slide order and visibility without
 * affecting content.
 *
 * Features include:
...
// Constants
const STORAGE_KEY = 'investorSlidesUserConfig';
const MAX_BACKUPS = 5;
...
class SlideService {
  constructor() {
    this.slides = [...investorSlidesConfig];
  ...
  createSection(newSectionName, slideIds = []) {
    ...
    // Calculate maximum order directly from slides rather than using helper
    const maxOrder = Math.max(...this.slides.map(slide => slide.order || 0));
  ...
  resetToDefault() {
    ...
    this.slides.forEach((slide, index) => {
      const defaultSlide = investorSlidesConfig.find(s => s.id === slide.id);
      if (defaultSlide) {
   ...
   * @param {number} draggedSlideId - The id of the slide being dragged
   * @param {number} targetSlideId - The id of the slide it's being dropped on
  ...
  reorderSlides(draggedSlideId, targetSlideId) {
    ...
    console.log(`Reordering slide ${draggedSlideId} to order ${newOrder}`, {
      before: [...this.slides].map(s => ({ id: s.id, order: s.order })),
    });

    // Make sure we save to localStorage
    const saveResult = this.saveUserConfig();

    // Verify the save worked
    if (saveResult) {
      console.log(`Saved slide reordering to localStorage successfully`);
    } else {
      console.error(`Failed to save slide reordering to localStorage`);
    }
    ...
    try {
      const savedConfig = localStorage.getItem(STORAGE_KEY);
      if (savedConfig) {
        console.log(`localStorage was updated, config length: ${savedConfig.length} bytes`);
      } else {
        console.error(`localStorage wasn't updated properly, null value returned`);
      }
    } catch (err) {
   ...
   * @returns {Array} Array of section objects with name and order properties
   */
  getSectionOrder() {
  ...
  /**
   * Reorder sections
   * @param {string} sectionToMove - The name of the section to move
   * @param {string} targetSection - The name of the target section (to position before/after)
