...
// Note: Calendar Events section has been removed since the floating calendar widget
// provides the same functionality in a more accessible way across the application.
import React, { useState, useEffect, useRef, useCallback } from 'react';
import { 
  AlertCircle, Activity, Users, Search, X, RefreshCw, 
...
const ChildrenTrackingTab = () => {
  ...
  // Local state
  const [activeSection, setActiveSection] = useState('allieDrive'); // 'allieDrive', 'growth', 'gifts', 'activities'
  ...
  // Add event listeners for provider updates with recursion protection
  useEffect(() => {
    // Flag to track if we're already in the process of handling an update
    ...
    const handleProviderChanged = () => {
      ...
      if (isHandlingUpdate) {
        console.log("Already handling provider update, skipping duplicate request");
        return;
      }
      ...
      if (activeChild) {
        ...
        setTimeout(() => {
          loadRelevantProviders(activeChild).finally(() => {
  ...
  const loadRelevantProviders = async (childId) => {
    try {
      ...
      try {
        ...
        querySnapshot.forEach((doc) => {
          ...
          if (!data.childId || data.childId === childId) {
            // Add provider if we don't already have it with the same name
            const existsWithName = providers.some(p => p.name === data.name);
            if (!existsWithName) {
              providers.push({
                id: doc.id,
                ...data,
      ...
      // CRITICAL FIX: If no providers found, check for test providers without family filter
      if (!providersFound) {
        console.log("No providers found with family filter, trying without filter for test providers");

        try {
          // Try providers collection without filter
          const providersCollection = collection(db, "providers");
          const querySnapshot = await getDocs(providersCollection);

          querySnapshot.forEach((doc) => {
            const data = doc.data();
            // For unfamilied providers, only include test providers
            if (data.name && data.name.toLowerCase().includes('test')) {
              providers.push({
                id: doc.id,
                ...data,
                familyId: familyId, // Assign current familyId
                childId: childId,   // Assign current childId
                source: "providers"
              });
            }
          });

          // Try familyProviders collection without filter
          const familyProvidersCollection = collection(db, "familyProviders");
          ...
          fpQuerySnapshot.forEach((doc) => {
            ...
            if (data.name && data.name.toLowerCase().includes('test')) {
              ...
              const existingProvider = providers.find(p => p.name === data.name);
              if (!existingProvider) {
                providers.push({
                  id: doc.id,
                  ...data,
          ...
          console.log(`Found ${providers.length} test providers without family filter`);
        } catch (error) {
          console.error("Error querying collections without family filter:", error);
        }
      }
  ...
  const loadRelevantDocuments = async (childId) => {
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        // Include if: specific to this child, or null/undefined childId (family-wide)
        if (!data.childId || data.childId === childId) {
          // Only add document if we don't already have it with the same name
          const existsWithName = documentsList.some(d => d.name === data.name);
          if (!existsWithName) {
            documentsList.push({
              id: doc.id,
              ...data
      ...
      setLoadingSection(null);
      return Promise.resolve(documentsList);
    } catch (error) {
    ...
    <div className="relative min-h-full">
        ...
        <div className="flex space-x-4">
          ...
          >
            <div className="flex items-center">
              ...
              Family Allie Drive
              <span className="ml-2 px-2 py-0.5 bg-yellow-100 text-yellow-700 text-xs rounded-full">Moved</span>
            </div>
          </button>
          <button
      ...
      {activeChild && (
        <div className="bg-white rounded-lg shadow-sm p-1">
          {/* Calendar Events Section removed - now available in floating calendar widget */}
          
          {/* Family Allie Drive Section - DEPRECATED: Use the dedicated Family Allie Drive tab instead */}
          {/* Commented out as this functionality has been moved to its own tab with inbox integration
          {activeSection === 'allieDrive' && (
            <div className="p-4">
              <div className="mb-6">
                <FamilyAllieDrive />
            ...
            <div className="p-4">
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">
                  Family Allie Drive Has Moved
                </h3>
                <p className="text-gray-600 mb-4">
                  The Family Allie Drive is now available as a dedicated tab with enhanced features 
                  including message inbox and email/SMS integration.
                </p>
                <button 
                  className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
                  onClick={() => {
                    // Navigate to the Family Allie Drive tab
                    // This would need to be implemented based on your navigation structure
                    alert('Please use the Family Allie Drive tab from the main navigation');
                  }}
                >
                  Go to Family Allie Drive
                </button>
              </div>
            </div>
          )}
      ...
      {/* Family Allie Drive content is handled by the FamilyAllieDrive component */}
      {/* We don't need to render the providers and documents sections separately here */}
    </div>
  );
};
