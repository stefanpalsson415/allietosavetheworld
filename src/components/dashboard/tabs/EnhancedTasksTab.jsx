...
// Removed custom avatar helpers - using shared UserAvatar component
...
const EnhancedTasksTab = ({ onStartWeeklyCheckIn, onOpenFamilyMeeting }) => {
  ...
  const { createCelebration: showNotification } = useNotification();
    ...
    try {
      ...
      if (eventDetails.eventId) {
        ...
        const eventUpdate = {
          ...
          // Preserve the user-selected category and eventType if available
      ...
      // Use our global notification system
      showNotification(`Cycle ${currentWeek} due date updated to ${formattedDate}`, "success");
      
      setIsProcessing(false);
      return true;
    } catch (error) {
      console.error("Error updating cycle due date:", error);
      showNotification("Failed to update due date. Please try again.", "error");
      setIsProcessing(false);
      return false;
    }
  };
  
  // Create a celebration notification - now redirects to use the global notification system
  const createCelebration = (habitTitle, success = true, customMessage = null) => {
    // Generate a message if one isn't provided
    const message = customMessage || generateCelebrationMessage(habitTitle);
    
    // Use our global notification system
    showNotification(
      success ? habitTitle : "Error", 
      success ? "success" : "error", 
      message
    );
    
    // If success, also launch confetti for a fun effect
    if (success) {
      launchConfetti();
    }
  };
          ...
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
            <EnhancedEventManager
              initialEvent={existingDueDateEvent ? {
                ...existingDueDateEvent,
                // Make sure the date is properly set
                dateTime: datePickerDate instanceof Date && !isNaN(datePickerDate.getTime()) 
                  ? datePickerDate.toISOString() 
                  : existingDueDateEvent.dateTime || new Date().toISOString()
              } : {
              ...
              onSave={async (event) => {
                try {
                  ...
                  const success = await updateCycleDueDate(newDate, {
                    ...
                    // Pass along the user-selected event type
