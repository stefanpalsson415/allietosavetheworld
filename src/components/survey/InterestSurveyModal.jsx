          ...
          <div className="text-center py-6">
            ...
            <div className="bg-gradient-to-r from-purple-50 to-indigo-50 p-6 rounded-xl mb-6 text-left shadow-sm border border-purple-100">
              ...
              <ul className="space-y-3">
                ...
                {calculateTopInterests(results).map((interest, index) => (
...
// Simply calculate top interests based on win counts
function calculateTopInterests(results, limit = 3) {
  ...
  results.forEach(result => {
    ...
    winCounts[result.loserId] = winCounts[result.loserId] || 0;
  });
  
  // Format results
  const interestWins = [];
  Object.entries(winCounts).forEach(([id, wins]) => {
    const result = results.find(r => r.winnerId === id || r.loserId === id);
    if (result) {
      const name = id === result.winnerId ? result.winnerName : result.loserName;
      interestWins.push({ id, name, wins });
    }
  });
