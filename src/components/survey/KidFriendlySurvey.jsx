...
import AllieChat from '../chat/refactored/AllieChat'; // Import AllieChat component
...
// Legacy simplification function - kept for backward compatibility
const legacySimplifyQuestionForChild = (question, childAge) => {
  if (!question) return "Which parent does this in your family?";
  ...
  if (childAge < 8) {
    ...
    if (category.includes("Household")) {
      if (originalText.includes("clean") || originalText.includes("dust")) {
        return "Which parent cleans up the house?";
      } else if (originalText.includes("cook") || originalText.includes("meal")) {
        return "Which parent makes food for everyone?";
      } else if (originalText.includes("shop") || originalText.includes("grocery")) {
        return "Which parent buys the food at the store?";
      } else if (originalText.includes("yard") || originalText.includes("garden")) {
        return "Which parent takes care of the yard?";
      } else if (originalText.includes("laundry") || originalText.includes("clothes")) {
        return "Which parent washes the clothes?";
      } else if (originalText.includes("plan")) {
        return "Which parent decides what your family is going to do?";
      } else if (originalText.includes("responsible for")) {
        return `Which parent ${originalText.toLowerCase().replace("who is responsible for", "does the")}?`;
      }
      // Default household simplification
      return `Which parent ${originalText.toLowerCase().replace("who ", "").replace("responsible for", "does")}?`;
    } 
    // Handle Parental Tasks - completely reframe these for children
      ...
      if (originalText.includes("primary responsibility")) {
        return "Which parent takes care of you the most?";
      } else if (originalText.includes("homework") || originalText.includes("school")) {
        return "Which parent helps you with your schoolwork?";
        ...
        return "Which parent drives you to places?";
      } else if (originalText.includes("emotion") || originalText.includes("feel")) {
        return "Which parent helps when you feel sad?";
      } else if (originalText.includes("bedtime")) {
        return "Which parent puts you to bed at night?";
      } else if (originalText.includes("coordinates")) {
        return "Which parent makes sure you get to all your activities?";
      } else if (originalText.includes("mental load") || originalText.includes("invisible")) {
        return "Which parent remembers all the important things in your family?";
      } else if (originalText.includes("anticipates")) {
        return "Which parent knows what you need before you ask?";
      }
      // Default parental simplification - properly reframe for child's perspective
      return `Which parent helps you ${originalText.toLowerCase().replace("who ", "").replace("responsible for", "with")}?`;
    }
  }
  // For older children (8-12)
    ...
    if (category.includes("Parental")) {
      if (originalText.includes("primary responsibility")) {
        return "Which parent usually takes care of you day-to-day?";
      } else if (originalText.includes("mental load") || originalText.includes("invisible")) {
        return "Which parent remembers all the important things for your family?";
      } else if (originalText.includes("coordinates")) {
        return "Which parent organizes your activities and schedule?";
      } else if (originalText.includes("anticipates")) {
        return "Which parent usually knows what you need before you ask?";
      } else if (originalText.includes("responsible for")) {
    ...
    // Add "Which parent" if it doesn't start with it
    if (!simplifiedText.startsWith("Which parent") && !simplifiedText.startsWith("Who")) {
      return `Which parent ${simplifiedText.toLowerCase()}?`;
    } else if (simplifiedText.startsWith("Who")) {
      return simplifiedText.replace(/^Who/, "Which parent");
    }
...
useEffect(() => {
  ...
  if (selectedUser && selectedUser.role === 'child') {
      ...
      // Optional: Use AI to batch process questions for even better simplification
      // This is commented out by default to avoid API calls during development
      // Uncomment to enable AI-enhanced simplification
      /*
      try {
        const aiSimplifiedMap = await KidQuestionSimplifier.batchSimplifyQuestions(
          questionSet.slice(0, 50), // Process first 50 questions
          childAge
        );
        
        // Apply AI simplifications
        questionSet = questionSet.map(q => {
          if (aiSimplifiedMap.has(q.id)) {
            return {
              ...q,
              aiSimplifiedText: aiSimplifiedMap.get(q.id)
            };
          }
          return q;
        });
      } catch (error) {
        console.error('Error with AI simplification, using standard simplification:', error);
      }
      */
      ...
      categories.forEach(category => {
        ...
        while (addedCount < questionsPerCategory && questionIndex < categoryQuestions.length) {
          ...
          // Use the new KidQuestionSimplifier for better results
          let simplifiedText;
          try {
            simplifiedText = KidQuestionSimplifier.simplifyQuestionForChild(originalQuestion, childAge, selectedUser?.name);
          } catch (error) {
            console.warn('KidQuestionSimplifier not available, using legacy simplification:', error);
            // Fallback to legacy simplification but ensure "Which parent" is used
            simplifiedText = legacySimplifyQuestionForChild(originalQuestion, childAge);
            // Always ensure questions start with "Which parent" instead of "Who"
            if (simplifiedText.startsWith('Who ')) {
              simplifiedText = simplifiedText.replace(/^Who /, 'Which parent ');
            }
          }
      ...
      <div className="bg-white rounded-xl p-6 shadow-md mb-6 border-2 border-gray-100">
        ...
        <div className="text-center mb-4">
          <h2 className="text-xl font-semibold text-black">
            {(() => {
              let questionText = currentQuestion.childText || currentQuestion.text;
              // Final safety check: ensure all questions use "Which parent" instead of "Who"
              if (questionText.startsWith('Who ')) {
                questionText = questionText.replace(/^Who /, 'Which parent ');
              }
              return questionText;
            })()} {renderIllustration()}
          </h2>
