...
const LocationsSettingsTab = () => {
  ...
  const getCategoryColor = (categoryId) => {
    const colors = {
      ...
      school: '#3B82F6', // blue
      doctor: '#10B981', // green
      pharmacy: '#F59E0B', // yellow
      grocery: '#8B5CF6', // purple
      park: '#059669', // emerald
      library: '#6366F1', // indigo
      sports: '#EC4899', // pink
      afterschool: '#F97316', // orange
      relatives: '#14B8A6', // teal
      workplace: '#6B7280' // gray
    };
    return colors[categoryId] || '#3B82F6'; // default to blue
  };
  ...
  const mapMarkers = React.useMemo(() => {
    ...
    savedLocations.forEach(loc => {
      if (loc.address && loc.lat && loc.lng) {
        const categoryInfo = DEFAULT_IMPORTANT_LOCATIONS.find(cat => cat.id === loc.id);
        markers.push({
          id: loc.id,
          lat: loc.lat,
          lng: loc.lng,
          title: categoryInfo ? categoryInfo.name : loc.name || loc.id,
          description: loc.address,
          color: getCategoryColor(loc.id),
          category: loc.id
        });
      }
    });
    ...
    return markers;
  }, [savedLocations, customLocations]);
    ...
    <div className="p-4 bg-white rounded-lg shadow">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          {!homeLocation ? (
            <div className="w-full h-[500px] rounded-lg border border-gray-200 flex items-center justify-center bg-gray-100">
            ...
            <GoogleMapView
              center={homeLocation ? { lat: homeLocation.lat, lng: homeLocation.lng } : { lat: 40.7128, lng: -74.0060 }}
              zoom={homeLocation ? 12 : 10}
              markers={mapMarkers}
              height="500px"
