      ...
      if (providers.length === 0 && familyId && !isLoadingProviders) {
        try {
          ...
          try {
            ...
            console.log(`Found ${allProviders.length} providers in "providers" collection`);
          } catch (error) {
          ...
          try {
            ...
            querySnapshot.forEach((doc) => {
              ...
              const existingProvider = allProviders.find(p => p.name === doc.data().name);
              if (!existingProvider) {
                allProviders.push({
                  id: doc.id,
                  ...doc.data(),
    ...
    if (editingProvider) {
      ...
      try {
        if (onAddProvider) {
          ...
          // Direct save to Firestore - ALWAYS to the "providers" collection
          const providersCollection = collection(db, "providers");
