...
const UserSignupScreen = () => {
  ...
  useEffect(() => {
    if (location?.state?.fromPayment && location?.state?.familyData) {
      ...
      // Set the received data - preserve ALL onboarding data
      setFamilyData({
        ...
        // Preserve all the onboarding preferences
        communication: receivedData.communication || {},
        preferences: receivedData.preferences || {},
        priorities: receivedData.priorities || {},
        aiPreferences: receivedData.aiPreferences || {}
      });
    }
  }, [location]);
  ...
  const validateForm = () => {
    ...
    // Validate parents - only name is required now
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Add email and temporary password to parents since DatabaseService expects them
      const familyDataForCreation = {
        ...familyData,
        parents: familyData.parents.map((parent, index) => {
          if (index === 0 && familyData.emailVerified && familyData.email) {
            // First parent gets the verified email and a temporary password
            return {
              ...parent,
              email: familyData.email,
              password: 'TempPass123!' // Temporary password that meets Firebase requirements
            };
          } else {
            // Other parents need email/password too for DatabaseService
            return {
              ...parent,
              email: parent.email || `${parent.name.toLowerCase().replace(/\s/g, '')}@${familyData.familyName.toLowerCase()}.family`,
              password: parent.password || 'TempPass123!'
            };
          }
        })
      };
      ...
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <div className="mb-8">
            ...
            {(familyData.emailVerified || familyData.phoneVerified) && (
              <div className="bg-green-50 p-4 rounded-lg mb-6 border border-green-200">
                <h3 className="text-sm font-medium text-green-900 mb-2">Verified Contact Information</h3>
                {familyData.emailVerified && familyData.email && (
                  <p className="text-sm text-green-800 flex items-center gap-2 mb-1">
                    <CheckCircle size={16} className="text-green-600" />
                    Email: {familyData.email}
                  </p>
                )}
                {familyData.phoneVerified && familyData.fullPhoneNumber && (
                  <p className="text-sm text-green-800 flex items-center gap-2">
                    <CheckCircle size={16} className="text-green-600" />
                    Phone: {familyData.fullPhoneNumber.startsWith('+46') ? 
                      familyData.fullPhoneNumber.replace(/(\+46)(\d{2})(\d{3})(\d{2})(\d{2})/, '$1 $2 $3 $4 $5') :
                      familyData.fullPhoneNumber}
                  </p>
                )}
                {familyData.familyEmailPrefix && (
                  <p className="text-sm text-green-800 flex items-center gap-2 mt-1">
                    <CheckCircle size={16} className="text-green-600" />
                    Family Email: {familyData.familyEmailPrefix}@families.checkallie.com
                  </p>
                )}
              </div>
            )}
