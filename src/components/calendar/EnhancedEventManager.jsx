...
const EnhancedEventManager = ({ 
  ...
  const { allieProcessor, notificationEngine, createEvent: unifiedCreateEvent, updateEvent: unifiedUpdateEvent, isInitialized } = useUnifiedEvent();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Use unified event system if available AND initialized
      if (isInitialized && unifiedCreateEvent && unifiedUpdateEvent) {
        setLoading(true);
        try {
          // Convert to calendar-v2 format
          const v2Event = {
            title: eventToSave.title,
            description: eventToSave.description || '',
            startTime: eventToSave.dateObj,
            endTime: eventToSave.dateEndObj,
            location: eventToSave.location,
            category: eventToSave.category || 'general',
            attendees: (eventToSave.attendees || []).map(memberId => {
            ...
            reminders: eventToSave.reminders || [{
              type: 'notification',
              minutesBefore: 15,
              message: `${eventToSave.title} starts in 15 minutes`
            }],
            recurrence: eventToSave.recurrence,
            metadata: {
              ...eventToSave.extraDetails,
              originalId: eventToSave.id || eventToSave.firestoreId
            }
          };

          let result;
          if (mode === 'edit' && (eventToSave.id || eventToSave.firestoreId)) {
            await unifiedUpdateEvent(eventToSave.id || eventToSave.firestoreId, v2Event);
            result = { ...v2Event, id: eventToSave.id || eventToSave.firestoreId };
          } else {
            result = await unifiedCreateEvent(v2Event);
          }

          // Also call original onSave for backward compatibility
          if (onSave) {
            await onSave(eventToSave, mode);
          }
          ...
          console.error("Error saving event with unified system:", error);
        ...
        // Use CalendarService directly when UnifiedEventContext not initialized or no onSave handler
        setLoading(true);
        try {
          console.log("Using CalendarService directly for event save");
          ...
          // Add required fields for CalendarService
          eventToSave.userId = userId;
          eventToSave.familyId = familyId;
          
          let result;
          if (mode === 'edit' && (eventToSave.id || eventToSave.firestoreId)) {
            console.log("Updating existing event:", eventToSave.id || eventToSave.firestoreId);
            result = await CalendarService.updateEvent(eventToSave.id || eventToSave.firestoreId, eventToSave);
          } else {
            console.log("Creating new event");
            result = await CalendarService.createEvent(eventToSave);
          }
          ...
          // After a delay, close the modal if onCancel is provided
          setTimeout(() => {
            if (onCancel) {
              onCancel();
            }
          ...
          console.error("Error saving event with CalendarService:", error);
        ...
        <div className="md:col-span-2 flex justify-between pt-2">
          ...
          {!isInitialized && (
            <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md flex items-center text-yellow-800 text-sm">
              ...
              <span>Please wait for the calendar to initialize before creating events.</span>
            </div>
          )}
          ...
          <div className="flex">
            ...
            <button
              ...
              title={!event.title ? 'Please enter a title' : ''}
            >
              {loading ? (
                <>
