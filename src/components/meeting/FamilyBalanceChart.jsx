    ...
    completedWeeks.forEach(week => {
      ...
      if (weekData) {
        ...
        data.push({
          ...
          habits: (weekData?.habits?.filter(h => h.completed)?.length || weekData?.tasks?.filter(t => t.completed)?.length || 0)
        });
      }
    });
  }
      ...
      return (
        <div className="bg-white p-4 border rounded shadow-lg">
          ...
          <p className="text-yellow-600 font-medium">
            Completed Habits: {payload[2].value}
          </p>
        </div>
      );
    }
    ...
    <div className="bg-white rounded-lg shadow p-4 h-64">
      ...
      <ResponsiveContainer width="100%" height="85%">
        <AreaChart data={data}>
          ...
          <Area type="monotone" dataKey="habits" stroke="#ffc658" fill="#ffc658" name="Completed Habits" />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
