...
class AllieAIService {
  constructor() {
    this.contextCache = {};
    this.feedbackLog = {};
    this._unifiedEventContext = null;
  }

  /**
   * Get the UnifiedEventContext from React context
   * This is set by components that use AllieAIService
   */
  setUnifiedEventContext(context) {
    this._unifiedEventContext = context;
  }

  getUnifiedEventContext() {
    return this._unifiedEventContext;
  }
...
safelyParseJSON(jsonString, defaultValue) {
  try {
    ...
    cleanedString = cleanedString.replace(/^Here is the JSON:?\s*/i, '');
    ...
    try {
      ...
      const objectMatch = cleanedString.match(/(\{[\s\S]*\})/);
...
async generateBalanceExperiments(familyId, weightedScores) {
  ...
  async getFamilyContext(familyId) {
    ...
    try {
      ...
      const subcategoryAnalysis = SubCategoryAnalyzer.analyzeSubCategories(surveyResponses);
      const mostImbalancedSubcategory = SubCategoryAnalyzer.getMostImbalancedSubcategory(subcategoryAnalysis);
      const topImbalancedSubcategories = SubCategoryAnalyzer.getTopImbalancedSubcategories(subcategoryAnalysis, 5);
      const subcategoryInsights = SubCategoryAnalyzer.generateSubcategoryInsights(subcategoryAnalysis);
      ...
      const context = {
        ...
        adults: (Array.isArray(members) ? members : []).filter(m => m.role === 'parent').length,
        children: (Array.isArray(members) ? members : []).filter(m => m.role === 'child'),
        relationship: {
        ...
        survey: {
          ...
          topImbalancedSubcategories: topImbalancedSubcategories,
...
async processProviderFromChat(message, familyId) {
  try {
    if (!familyId) {
      ...
      // Try to find familyId from user context or localStorage
      if (typeof window !== 'undefined') {
        ...
        if (familyId) {
          ...
          // Last resort fallback to known ID
    ...
    // CRITICAL: Log operation with clear visibility
    console.log(`🔴 CREATING PROVIDER: "${providerDetails.name}" for family ${familyId}`);

    // CRITICAL FIX: Add timestamp to help track and debug
    const timestamp = new Date().toISOString();
    providerDetails.createdAt = timestamp;
    providerDetails.notes = (providerDetails.notes || "") +
        `\n[Created via AllieChat at ${timestamp}]`;

    // SIMPLIFIED: Use a clean approach - import the service explicitly with error handling
    let ProviderService;
    try {
      const module = await import('./ProviderService');
      ProviderService = module.default;
      console.log("✅ Successfully imported ProviderService");
    } catch (importError) {
      console.error("❌ Failed to import ProviderService:", importError);
      return { success: false, error: "System error: Could not load provider service" };
    }

    // Verify Provider Service available
    if (!ProviderService || !ProviderService.saveProvider) {
      console.error("❌ ProviderService or saveProvider method not available");
      return { success: false, error: "System error: Provider service unavailable" };
    }

    // Save provider using the service with explicit error handling
    console.log("🔄 Calling ProviderService.saveProvider with:", {
      familyId,
    ...
    let result;
    try {
      result = await ProviderService.saveProvider(familyId, providerDetails);
      console.log("✅ Provider save result:", result);
    } catch (saveError) {
      console.error("❌ Error in ProviderService.saveProvider:", saveError);
      return {
        success: false,
        error: "I encountered an error while saving the provider. Technical details: " + saveError.message
      };
    }
    ...
    if (result && result.success) {
      ...
      try {
        if (typeof window !== 'undefined') {
          ...
          // Multiple refreshes with delays for extra reliability
          setTimeout(() => {
        ...
        console.warn("⚠️ Non-critical event dispatch error:", eventError);
      ...
      console.error("❌ Provider save operation failed:", result?.error || "Unknown error");
      return {
        success: false,
        error: (result && result.error) || "I encountered an error while trying to save this provider. Please try again."
      };
    }
  } catch (error) {
    console.error("❌ Error processing provider request:", error);

    // Better error recovery with more detailed diagnostics
    if (error.message?.includes("permission") || error.code === "permission-denied") {
      console.error("🔒 Permission error detected when adding provider");
      return {
        success: false,
...
async processBabysitterRequest(message, familyId, childId = null) {
  try {
    ...
    // Process as a regular provider creation request
    const enhancedMessage = `Add babysitter ${providerData.name} to our providers. ${providerData.notes}`;
    return await this.processProviderFromChat(enhancedMessage, familyId);
  } catch (error) {
...
async createOrFindProvider(familyId, name, specialty, email, phone) {
  try {
    ...
    // CRITICAL FIX: Use "familyProviders" collection to match what the UI queries
...
async processEventFromChat(message, familyId, eventType = null, childId = null) {
  try {
    ...
    // Check if UnifiedEventContext is available
    const unifiedContext = this.getUnifiedEventContext();
    
    let eventDetails;
    if (unifiedContext && unifiedContext.allieProcessor) {
      // Use the new AllieEventProcessor from calendar-v2
      eventDetails = await unifiedContext.allieProcessor.processNaturalLanguage(message);
    } else {
      // Fallback to old parser
      const UnifiedParserService = (await import('./UnifiedParserService')).default;
      eventDetails = await UnifiedParserService.parseEvent(message, familyContext);
    }
    ...
    // Use UnifiedEventContext if available, otherwise fallback to old service
    if (unifiedContext && unifiedContext.createEvent) {
      // Use the new unified event creation
      const eventId = await unifiedContext.createEvent({
        ...completeEvent,
        metadata: {
          ...completeEvent.metadata,
          source: 'ai-service',
          messageSource: message.substring(0, 100)
        }
      });
      ...
      // Fallback to old UnifiedEventService
...
async processTaskFromChat(message, familyId, userId) {
  try {
    ...
    if (taskDetails.assignedTo) {
      ...
      // familyMembers might be an object - convert to array first
      const familyMembersRaw = familyContext.familyMembers || [];
      const familyMembers = typeof familyMembersRaw === 'object' && !Array.isArray(familyMembersRaw)
        ? Object.values(familyMembersRaw)
        : familyMembersRaw;
      const matchedMember = (Array.isArray(familyMembers) ? familyMembers : []).find(member =>
        ...
        try {
          ...
          const calendarEvent = {
            ...
            assignedToName: task.assignedTo ? (() => {
              // familyMembers might be an object - convert to array first
              const members = typeof familyContext.familyMembers === 'object' && !Array.isArray(familyContext.familyMembers)
                ? Object.values(familyContext.familyMembers)
                : familyContext.familyMembers;
              return (Array.isArray(members) ? members : []).find(m => m.id === task.assignedTo)?.name;
            })() : null,
            reminders: {
              useDefault: true
            }
...
getFallbackChildInsights(childrenData) {
  ...
  async generatePersonalizedTasks(familyId, currentWeek, previousTasks = []) {
    try {
      ...
      // familyMembers might be an object - convert to array first
      const familyMembersForPrompt = typeof familyData.familyMembers === 'object' && !Array.isArray(familyData.familyMembers)
        ? Object.values(familyData.familyMembers)
        : familyData.familyMembers;
      ...
      Parents: ${JSON.stringify((Array.isArray(familyMembersForPrompt) ? familyMembersForPrompt : []).filter(m => m.role === 'parent'))}
      Children: ${JSON.stringify((Array.isArray(familyMembersForPrompt) ? familyMembersForPrompt : []).filter(m => m.role === 'child'))}
      Current Week: ${currentWeek}
      
      Use this information to generate 4 specific tasks for Week ${currentWeek}:
      1. Create 2 tasks for each parent
      2. Focus on the specific SUBCATEGORIES with the highest imbalance (not just categories)
      3. Avoid repeating tasks from previous weeks
      4. Each task should have 2-3 specific subtasks
      5. Include an AI insight explaining why each task was selected based on subcategory imbalance
      6. Include the specific subcategory being addressed in each task
        ...
        "tasks": [
          {
            ...
            "subcategory": "string (specific subcategory id, e.g., 'worrying', 'meal_planning')",
            "subcategoryLabel": "string (subcategory label, e.g., 'Worrying About Kids')",
            "aiInsight": "string (explanation of why this task matters based on subcategory imbalance)",
            "isAIGenerated": true,
            "subTasks": [
              {
      ...
      Most Imbalanced: ${familyContext.survey.mostImbalancedSubcategory ? 
        `${familyContext.survey.mostImbalancedSubcategory.subcategoryLabel} (${familyContext.survey.mostImbalancedSubcategory.imbalancePercent}% imbalance)` : 
        'No subcategory data'}
      Top 5 Imbalanced Subcategories:
      ${familyContext.survey.topImbalancedSubcategories ? 
        familyContext.survey.topImbalancedSubcategories.map(sub => 
          `- ${sub.subcategoryLabel}: ${sub.imbalancePercent}% imbalance (${sub.dominantRole} doing more)`
        ).join('\n') : 'No subcategory data'}
      ...
      IMPORTANT: Focus tasks on the most imbalanced subcategories identified above!
...
getFallbackMeetingAgenda(cycleNumber) {
   ...
   * @param {object} params - Parameters for generating insights
   * @returns {Promise<Array>} Array of insights
   */
  async generateInsights(params) {
    const { familyId, balanceMetrics, taskRecommendations, surveyResponses, weekHistory, completedWeeks, type, children, focus } = params;
    
    try {
      // Handle sibling dynamics insights specifically
      if (type === 'sibling-dynamics') {
        return await this.generateSiblingDynamicsInsights(familyId, children, focus);
      }
      ...
      const context = {
        ...
        recentSurveyData: surveyResponses || [],
        weeklyTrends: weekHistory || [],
        completedWeeks: completedWeeks || 0
      };

      const prompt = `Given this family's current balance metrics and history:
        - Mental Load Balance: ${balanceMetrics?.mentalBalance || 50}%
        - Physical Load Balance: ${balanceMetrics?.physicalBalance || 50}%
        - Emotional Load Balance: ${balanceMetrics?.emotionalBalance || 50}%
        - Overall Balance: ${balanceMetrics?.overallBalance || 50}%
        
        Generate 3-5 actionable insights to help improve their family balance. Focus on:
        1. What's working well
        2. Areas needing attention
        3. Specific recommendations for improvement
        
        Return as JSON array with format: { title, content, type, priority }`;

      const response = await ClaudeService.sendMessage(prompt, 'assistant');
      
      // Parse the response
      const insights = this.safelyParseJSON(response, { insights: [] });
      ...
      if (Array.isArray(insights)) {
        ...
        // Fallback insights
        return this.generateFallbackInsights(balanceMetrics);
      }
    } catch (error) {
   ...
   * @param {string} familyId - Family ID
   * @param {Array} children - Array of children with id, name, age
   * @param {string} focus - Focus area for insights
   * @returns {Promise<Array>} Array of sibling dynamics insights
   */
  async generateSiblingDynamicsInsights(familyId, children, focus) {
    try {
      if (!children || children.length < 2) {
        return [{
          title: "Add More Children",
          content: "Sibling dynamics insights require at least two children in your family profile.",
          type: "info",
          priority: "medium"
        }];
      }
      
      const prompt = `Generate 3 specific suggestions for improving sibling collaboration in a family with these children:
        ${children.map(c => `- ${c.name || 'Child'}, age ${c.age || 'unknown'}`).join('\n')}
        
        Focus on ${focus || 'collaboration opportunities'} that:
        1. Encourage positive interactions between siblings
        2. Reduce parental supervision needs
        3. Build life skills through peer learning
        4. Are age-appropriate and engaging
        
        Return as JSON array with format: { title, content, type: "recommendation", priority: "high" }
        Each suggestion should be specific and actionable.`;
      
      const response = await ClaudeService.sendMessage(prompt, 'assistant');
      const insights = this.safelyParseJSON(response, []);
      ...
      // Fallback sibling suggestions
      return this.generateFallbackSiblingInsights(children);
    } catch (error) {
  ...
  /**
   * Generate fallback sibling insights
   * @param {Array} children - Array of children
   * @returns {Array} Fallback sibling insights
   */
  generateFallbackSiblingInsights(children) {
    const insights = [];
    
    // Age-based suggestions
    const ages = children.map(c => c.age).filter(Boolean);
    const hasTeens = ages.some(age => age >= 13);
    const hasYoungKids = ages.some(age => age <= 7);
    
    if (hasTeens && hasYoungKids) {
      insights.push({
        title: "Mentorship Program",
        content: "Set up a weekly 'big sibling teaches' session where older children teach younger ones a skill like drawing, reading, or simple games. This builds leadership skills and reduces your teaching load.",
        description: "Create structured teaching moments between older and younger siblings",
        type: "recommendation",
        priority: "high"
      });
    }
    ...
    insights.push({
      title: "Collaborative Chore Teams",
      content: "Pair siblings for household tasks like setting the table or organizing toys. Rotate pairs weekly to build different relationships and make chores more fun.",
      description: "Transform individual chores into team activities",
      type: "recommendation",
      priority: "high"
    });
    
    insights.push({
      title: "Sibling Game Time",
      content: "Schedule 30 minutes of daily 'sibling-only' play time where children play together without parent involvement. Start with 15 minutes and gradually increase.",
      description: "Foster independent play and problem-solving between siblings",
      type: "recommendation",
      priority: "high"
    });
   ...
   * @param {object} balanceMetrics - Current balance metrics
   * @returns {Array} Fallback insights
   */
  generateFallbackInsights(balanceMetrics) {
    const insights = [];
    
    // Add insights based on balance metrics
    if (balanceMetrics?.overallBalance < 40) {
      insights.push({
        title: "Balance Alert",
        content: "Your family's workload appears imbalanced. Consider redistributing tasks to improve overall harmony.",
        type: "alert",
        priority: "high"
      });
    }
    
    if (balanceMetrics?.mentalBalance < 50) {
      insights.push({
        title: "Mental Load Support",
        content: "The mental load seems heavy. Try delegating planning tasks and using shared calendars to distribute cognitive work.",
        type: "recommendation",
        priority: "medium"
      });
    }
    
    if (balanceMetrics?.physicalBalance > 70) {
      insights.push({
        title: "Physical Balance Success",
        content: "Great job on maintaining physical task balance! Keep up the good teamwork.",
        type: "success",
        priority: "low"
      });
    }
    ...
    insights.push({
      title: "Weekly Tip",
      content: "Regular family meetings help maintain balance. Schedule 15 minutes this week to review tasks together.",
      type: "tip",
      priority: "low"
    });
