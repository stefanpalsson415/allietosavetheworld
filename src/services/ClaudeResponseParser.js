...
class ClaudeResponseParser {
  ...
  static parseProviderInfo(text) {
    ...
    // IMPROVED: Save provider using the properly fixed window.createProviderFromAllie function
    try {
      // Create provider with extracted data
      const providerData = {
        name,
        type,
        email,
        ...
        isProvider: true,
        parsed: true,
        specialty: type || 'General' // Set specialty based on type if present
      };

      // Use the fixed dual-collection provider creation function if available
      if (typeof window !== 'undefined' && window.createProviderFromAllie) {
        console.log("⚡ Creating provider using improved dual-collection function:", providerData);
        window.createProviderFromAllie(providerData)
          .then(result => {
            if (result.success) {
              console.log("✅ Provider created successfully:", {
                id: result.id,
                savedToFirestore: result.savedToFirestore
              });
            } else {
              console.error("❌ Provider creation failed:", result.error);
            }
          })
          .catch(err => console.error("❌ Provider creation error:", err));
      }
      // Fallback to previous DIY approach in case window.createProviderFromAllie isn't available yet
      else if (typeof window !== 'undefined' && typeof localStorage !== 'undefined') {
        console.log("⚠️ Using fallback provider creation - window.createProviderFromAllie not found");
        try {
          // Create a unique ID
          const providerID = 'parser-' + Date.now() + '-' + Math.floor(Math.random() * 10000);

          // Get family ID with fallbacks - same approach as the fixed provider-fix.js
          ...
          const provider = {
            ...
            notes: "Added by Allie Chat via Claude parser"
          };
          ...
          console.log("✅ Parser directly saved provider to localStorage:", provider);

          // Try to update UI
          if (typeof window.dispatchEvent === 'function' && typeof CustomEvent === 'function') {
            try {
              window.dispatchEvent(new CustomEvent('provider-added', {
                detail: { provider, providerId: provider.id }
              }));
          ...
          console.error("❌ Error in direct localStorage saving:", directError);
        }
      }
    } catch (saveError) {
