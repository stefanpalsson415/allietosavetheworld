...
class DocumentOCRService {
    ...
    async imageTextExtraction(file, options) {
      try {
        ...
        const imageDataUrl = await DocumentOCRService.prototype.fileToDataUrl(file);
    ...
    /**
     * PDF text extraction with image conversion capability
    ...
    async pdfTextExtraction(file, options) {
      try {
        ...
        try {
          // Try to dynamically import PDF.js (v5.x uses different path)
          ...
          // Process multiple pages - limit to 10 pages for performance
          ...
          for (let i = 1; i <= maxPages; i++) {
            ...
            if (pageText.length > 50) {
              ...
              // If no text or minimal text, convert page to image
              console.log(`Page ${i} has minimal text (${pageText.length} chars), converting to image`);
              ...
              try {
                ...
                console.log(`Page ${i} converted to image (${canvas.width}x${canvas.height})`);
                
              } catch (renderError) {
                console.error(`Error rendering page ${i} to image:`, renderError);
              }
            }
          }
          ...
          // If we have page images but no extracted text, it's likely a scanned PDF
          if (!hasExtractableText && pageImages.length > 0) {
            console.log('PDF appears to be scanned or image-based, images extracted for OCR');
            confidence = 0.7; // Lower confidence for scanned PDFs
          } else if (hasExtractableText) {
            confidence = 0.95; // Higher confidence for text-based PDFs
          }
          
        } catch (pdfError) {
          console.log("PDF.js error occurred:", pdfError);
          
          // Fallback to basic content extraction
          text = `Content from PDF document: ${file.name}\n\nThis document appears to contain text content that we're unable to fully extract.`;
          pageCount = 1;
          confidence = 0.6;
        }
        ...
        return {
          success: true,
          text,
          pageImages, // Include extracted images for further processing
          metadata: {
