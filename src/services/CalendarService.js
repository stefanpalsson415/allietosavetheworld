...
// Removed circular dependency: EventStore will be imported dynamically
...
class CalendarService {
  ...
  // Lazy getter for EventStore to avoid circular dependency
  async getEventStore() {
    if (!this._eventStore) {
  ...
  async loadUserCalendarSettings(userId) {
    try {
      ...
      // Return default settings as fallback for permission errors
      ...
      this.calendarSettings = fallbackSettings;
  ...
  async saveUserCalendarSettings(userId, settings) {
    try {
      ...
      await setDoc(docRef, { calendarSettings: settings }, { merge: true });
  ...
  async addEvent(event, userId, familyId) {
    try {
      ...
      console.log("Adding calendar event:", {
        ...
        familyId: familyId || event.familyId
      });
      ...
      // Check for relevant places and add location if found
      try {
        const QuantumKnowledgeGraph = (await import('./QuantumKnowledgeGraph')).default;
        const relevantPlaces = await QuantumKnowledgeGraph.getRelevantPlacesForEvent(
          familyId || event.familyId,
          eventData
        );
        
        if (relevantPlaces.length > 0) {
          const topPlace = relevantPlaces[0];
          console.log(`🌍 Found relevant place for event: ${topPlace.place.properties.name} (relevance: ${topPlace.relevance})`);
          
          // Add location to event if not already set
          if (!eventData.location && topPlace.relevance >= 0.8) {
            eventData.location = topPlace.place.properties.address;
            eventData.placeId = topPlace.place.id;
            eventData.placeName = topPlace.place.properties.name;
            console.log(`✅ Auto-added location: ${eventData.location}`);
          }
        }
      } catch (placeError) {
        console.warn('Could not check for relevant places:', placeError);
        // Continue anyway - this is an enhancement, not required
      }
      
      // Use the EventStore to add the event (with enhanced metadata)
      // Pass familyId as third parameter
      const eventStore = await this.getEventStore();
      const result = await eventStore.addEvent(eventData, userId, familyId || event.familyId);
  ...
  processEventDetailsForStorage(event) {
    ...
    const standardEvent = {
      ...
      // Check for startDate/endDate (used by UnifiedInbox) first
      start: event.start || {
        dateTime: event.startDate || event.startTime || event.dateTime || new Date().toISOString(),
        timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone
      },
      end: event.end || {
        dateTime: event.endDate || event.endTime || event.endDateTime || 
                  new Date(new Date(event.startDate || event.startTime || event.dateTime || Date.now()).getTime() + 60 * 60 * 1000).toISOString(),
        timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone
      },
  ...
  async getEvents(familyId, startDate, endDate) {
    try {
      ...
      // Use eventStore to get events
      const eventStore = await this.getEventStore();
      const events = await eventStore.getEventsByDateRange(
        familyId,
        startDate || new Date(),
