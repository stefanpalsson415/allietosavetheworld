...
async saveMessage(message) {
  try {
    ...
    console.log("ChatPersistenceService.saveMessage called with:", {
      ...
      threadId: message.threadId || "will be generated",
      parentMessageId: message.parentMessageId || "none"
    });
    ...
    // Generate message ID first so we can use it for threadId if needed
    const messageId = message.id || doc(collection(db, "chatMessages")).id;
    ...
    const enhancedMessage = {
      ...
      content: message.text, // Also save as content for MessageService compatibility
      sender: message.sender || "unknown",
      userId: message.userId || message.sender || "unknown",
      // CRITICAL: Ensure userName is set for proper avatar display
      userName: message.userName || (message.sender === 'allie' ? 'Allie' : 'User'),
      userAvatar: message.userImage || message.userAvatar || null,
      ...
      // Add support for Allie messages
      isFromAllie: message.sender === 'allie' || message.isFromAllie || false,
    ...
    while (retryCount < maxRetries && !docRef) {
      try {
        // Use setDoc instead of addDoc so we can specify the ID
        docRef = doc(db, "chatMessages", messageId);
        await setDoc(docRef, enhancedMessage);
      } catch (saveError) {
        retryCount++;
    ...
    console.log(`Message successfully saved with ID: ${messageId}, threadId: ${enhancedMessage.threadId}`);
    ...
    return { success: true, messageId: messageId, threadId: enhancedMessage.threadId };
  } catch (error) {
...
async deleteMessage(messageId, familyId) {
  ...
  async loadMessages(familyId, options = {}) {
    try {
      ...
      const { 
        pageSize = 100,  // Increased from 25 to 100 to load more messages
        loadMore = false,
      ...
      querySnapshot.forEach(doc => {
        ...
        // Store all messages for counting replies
        allMessagesMap.set(doc.id, {
          id: doc.id,
          ...data,
          timestamp: timestamp.toISOString(),
          sender: data.sender || 'unknown',
          userName: data.userName || (data.sender === 'allie' ? 'Allie' : 'User')
        });
        
        // Count replies for each thread
        if (data.parentMessageId) {
          // This is a reply, count it for its thread
          const threadId = data.threadId || data.parentMessageId;
          threadReplyCounts.set(threadId, (threadReplyCounts.get(threadId) || 0) + 1);
        }
      });
      
      // Second pass: build the main chat messages with reply counts
      allMessagesMap.forEach((messageData, messageId) => {
        // Only include messages that are NOT thread replies in the main chat
        if (!messageData.parentMessageId) {
          // Add reply count to the message
          const threadId = messageData.threadId || messageId;
          messageData.replyCount = threadReplyCounts.get(threadId) || 0;
