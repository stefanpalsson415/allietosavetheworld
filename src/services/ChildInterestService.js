...
class ChildInterestService {
  ...
  async getChildInterests(familyId, childId) {
    try {
      // Check if using new subcollection pattern
      const interestsCollectionRef = collection(db, "families", familyId, "childInterests", childId, "interests");
      const interestsSnapshot = await getDocs(interestsCollectionRef);

      if (!interestsSnapshot.empty) {
        // Return interests from subcollection (new pattern)
        const interests = [];
        interestsSnapshot.forEach(doc => {
          interests.push({ ...doc.data(), id: doc.id });
        });
        return interests;
      }
  ...
  async getClassifiedInterests(familyId, childId) {
    try {
      ...
      // If we have less than 3 interests or none have ratings yet,
      // put everything in uncategorized
      if (interests.length < 3 || !interests.some(i => i.rating && i.rating > 0)) {
  ...
  async addInterest(familyId, childId, interest) {
    try {
      // Use subcollection pattern instead of arrays for better scalability and reliability
      const interestsCollectionRef = collection(db, "families", familyId, "childInterests", childId, "interests");
      ...
      let initialRating = interest.elo || this.BASE_ELO;
      ...
      const newInterest = {
        ...
        dateAdded: serverTimestamp(), // Now we can use serverTimestamp in document
        rating: initialRating,
        comparisons: interest.comparisons || 0,
        uncertainty: interest.uncertainty || 350, // High initial uncertainty (Glicko-inspired)
        parentNotes: interest.parentNotes || '',
        source: interest.source || 'manual', // manual, survey, suggestion
        specifics: interest.specifics || {},
        history: [],
        // Add new fields for the 2.0 experience
        loved: interest.loved || false,
        liked: interest.liked || false,
        passed: interest.passed || false,
        lastInteraction: serverTimestamp(),
        interactionCount: 1
      };

      // Create document in subcollection
      const docRef = doc(interestsCollectionRef, interestId);
      await setDoc(docRef, newInterest);

      // Update parent document metadata
      const parentDocRef = doc(db, "families", familyId, "childInterests", childId);
      ...
      if (!parentDocSnap.exists()) {
        ...
        await setDoc(parentDocRef, {
          ...
          lastSurveyDate: null,
          totalInterests: 1,
          lastAddedInterest: interest.name
        });
      } else {
        // Update existing parent document
        await updateDoc(parentDocRef, {
          lastUpdated: serverTimestamp(),
          totalInterests: (parentDocSnap.data().totalInterests || 0) + 1,
          lastAddedInterest: interest.name
        });
      }
      ...
      if (error.code === 'permission-denied') {
        console.error("Permission denied - check Firestore rules for childInterests collection");
      }
      throw error;
    }
  }
   ...
   * @param {string} familyId - The family ID
   * @param {string} childId - The child ID
   * @param {string} interestId - The interest ID to update
   * @param {Object} updates - The fields to update
   * @returns {Promise<void>}
   */
  async updateInterest(familyId, childId, interestId, updates) {
    try {
      const interestRef = doc(db, "families", familyId, "childInterests", childId, "interests", interestId);
