...
import MultimodalUnderstandingService from './MultimodalUnderstandingService';
 ...
 * Integrates with MultimodalUnderstandingService for advanced multimodal processing
 */
class UnifiedParserService {
  constructor() {
    this.claudeService = ClaudeService;
    this.multimodalService = MultimodalUnderstandingService;
  }
...
Return ONLY valid JSON without any markdown code blocks or extra text:
{
  ...
  /**
   * Parse image content for specific information type (events, documents, etc.)
   * @param {File} imageFile - The image file to process
   * @param {string} type - The type of information to extract ('event', 'document', etc.)
   * @param {object} context - Additional context (family members, etc.)
   * @returns {Promise<object>} Extracted structured information from the image
   */
  async parseImage(imageFile, type = 'event', context = {}) {
    try {
      console.log(`🖼️ [ENHANCED] Processing ${type} information from image: ${imageFile.name}`);
      
      // Validate input file with detailed logging
      if (!imageFile) {
        console.error('❌ Image file is null or undefined');
        throw new Error('Invalid image file: File is missing');
      }
      
      if (!imageFile.name) {
        console.error('❌ Image file has no name property:', imageFile);
        // Continue anyway since this might just be a naming issue
        console.log('⚠️ Attempting to continue without filename');
      }
      
      if (!imageFile.type) {
        console.error('❌ Image file has no type property:', imageFile);
        // Try to infer type from name if possible
        const inferredType = imageFile.name?.toLowerCase().endsWith('.png') ? 'image/png' : 
                             imageFile.name?.toLowerCase().endsWith('.jpg') ? 'image/jpeg' :
                             imageFile.name?.toLowerCase().endsWith('.jpeg') ? 'image/jpeg' : 'image/png';
        console.log(`⚠️ Using inferred image type: ${inferredType}`);
        imageFile.type = inferredType;
      }
      
      // Use the new MultimodalUnderstandingService for advanced processing
      console.log(`🧠 Using MultimodalUnderstandingService for advanced processing...`);
      ...
      // Process the image using the multimodal service
      const multimodalResult = await this.multimodalService.processFile(
        imageFile,
        type === 'event' ? 'event' : 
          type === 'medical' ? 'medical' : 
          type === 'document' ? 'document' : 'general',
        enhancedContext
      );
      ...
      if (multimodalResult.success) {
        ...
        // Extract the relevant analysis data from the multimodal result
        const extractedData = multimodalResult.results?.analysis?.data || {};
        
        // Add metadata about the source being an image
        const result = {
          ...extractedData,
          source: 'multimodal_pipeline',
          originalFile: imageFile.name || 'unnamed_image',
          fileType: imageFile.type || 'unknown',
          processingTimestamp: new Date().toISOString(),
          confidence: multimodalResult.results?.analysis?.confidence || 0.8
        };
        
        console.log(`📊 Final result keys: ${Object.keys(result).join(', ')}`);
        return result;
      } else {
        // If multimodal service fails, fall back to the legacy method
        console.log(`⚠️ Multimodal processing failed, falling back to legacy method...`);
        
        // Legacy implementation (shortened for brevity)
        // Convert image to base64 for processing with better error handling
        const base64Image = await this.fileToBase64(imageFile);
        
        // Create a system prompt that includes foreign language translation
        const systemPrompt = `You are Allie, a family assistant specialized in extracting information from images.
        
        TASK:
        1. First IDENTIFY THE LANGUAGE in the image. If it's not English (especially if it's Swedish), translate all text to English.
        2. Then extract structured ${type} information from the image.
        
        ${this.getTypeSpecificImageInstructions(type, context)}
        
        YOU MUST RETURN A VALID JSON OBJECT.`;
        
        // Prepare messages for Claude with robust handling
        ...
        const messages = [
          {
            ...
            content: [
              {
                type: 'text',
                text: `Analyze this image and extract ${type} information.`
              },
              {
                type: 'image',
                source: {
        ...
        // Process the textual response
        const extractedData = this.processImageResponse(response, type);
        
        // Add metadata about the source
        const result = {
          ...extractedData,
          source: 'legacy_image_processing',
          originalFile: imageFile.name || 'unnamed_image',
          fileType: imageFile.type || 'unknown',
          processingTimestamp: new Date().toISOString()
        };
      ...
      console.error(`❌ Error parsing ${type} from image:`, error);
      
      // Return error object
      return {
        error: true,
        errorMessage: error.message || 'Unknown error processing image',
        errorType: error.name,
        source: 'image_processing_error',
        originalFile: imageFile.name || 'unnamed_image',
        fileType: imageFile.type || 'unknown',
        processingTimestamp: new Date().toISOString()
      };
  ...
  /**
   * Get type-specific instructions for image processing
   * @param {string} type - The type of information to extract
   * @param {object} context - Additional context
   * @returns {string} Type-specific instructions
   */
  getTypeSpecificImageInstructions(type, context = {}) {
    switch(type.toLowerCase()) {
      case 'event':
        return `Extract comprehensive invitation details:
        
        1. FOREIGN LANGUAGE HANDLING:
           - Translate any non-English text completely to English
           - Convert 24-hour times to standard format (e.g., 14:00 to 2:00 PM)
           - Format dates as MM/DD/YYYY for consistency
        
        2. EXTRACT AS JSON WITH THESE FIELDS:
           - title: Complete event title/name
           - eventType: The type of event (birthday, party, meeting, etc.)
           - dateTime: ISO format date/time when event starts
           - endDateTime: ISO format for when event ends (if specified)
           - location: Complete address or venue information
           - description: Comprehensive description including all important details
           - hostName: Who is hosting the event
           - contactInfo: Any RSVP phone numbers or emails
           - specialInstructions: Any specific requests (gifts, dress code, etc.)
           - originalLanguage: The language the invitation was in
           - translationNotes: Any important notes about the translation
        
        EXAMPLE JSON:
        {
          "title": "Emma's 8th Birthday Party",
          "eventType": "birthday",
          "dateTime": "2025-05-15T14:00:00.000Z",
          "endDateTime": "2025-05-15T16:00:00.000Z",
          "location": "123 Sunshine Lane, Stockholm",
          "description": "Pool party with cake and games",
          "hostName": "Emma's Parents (Maria & Johan)",
          "contactInfo": "+46 70 123 4567",
          "specialInstructions": "Bring swimsuit and towel. No gifts necessary.",
          "originalLanguage": "Swedish",
          "translationNotes": "Converted from 24-hour time format"
        }`;
        
      case 'document':
        return `Extract document information from the image...[document instructions]`;
        
      default:
        return `Extract all visible information from the image and convert to JSON format.`;
    }
  }

  /**
   * Process the response from Claude for image analysis
   * @param {string} response - Claude's response
   * @param {string} type - The type of information being extracted
   * @returns {object} Structured data
   */
  processImageResponse(response, type) {
    try {
      console.log(`Processing image response for ${type}, response length: ${response?.length || 0}`);
      
      if (!response || typeof response !== 'string') {
        console.error('Invalid response provided to processImageResponse');
        throw new Error('Invalid response from image analysis');
      }
      
      // First, look for any translation notes before JSON
      let translationNotes = null;
      let processedResponse = response;
      
      // Look for translation notes pattern
      const translationMatch = response.match(/Translation notes:\s*([\s\S]*?)\s*({[\s\S]*})/i);
      if (translationMatch) {
        translationNotes = translationMatch[1].trim();
        processedResponse = translationMatch[2]; // Update response to just contain the JSON part
        console.log('Found translation notes:', translationNotes?.substring(0, 100));
      }
      
      // Look for language identification pattern
      const languageMatch = response.match(/Language identified:\s*([^\n.]+)/i);
      if (languageMatch && languageMatch[1]) {
        const language = languageMatch[1].trim();
        translationNotes = translationNotes ? 
          `${translationNotes}\nLanguage identified: ${language}` : 
          `Language identified: ${language}`;
        console.log('Found language identification:', language);
      }
      
      // Extract JSON using existing method with improved error handling
      console.log('Extracting structured data from response...');
      const extractedData = this.processResponse(processedResponse, type);
      
      if (!extractedData) {
        console.log('First attempt at extracting data failed, trying alternative extraction approach');
        // Try an alternative extraction approach - looking for any JSON-like structure
        ...
        if (matches && matches.length > 0) {
          ...
          for (const match of matches) {
            try {
              ...
              // Verify this is a meaningful object with at least some expected fields
              if (data && typeof data === 'object' && (data.title || data.eventType || data.dateTime)) {
                console.log('Found valid JSON data in alternative extraction');
                
                // Add translation notes if found
                if (translationNotes) {
                  data.translationNotes = translationNotes;
                }
        ...
        throw new Error('Could not extract structured data from image response');
      }
      
      // Add translation notes if found
      if (translationNotes && extractedData) {
        extractedData.translationNotes = translationNotes;
        
        // If language was detected, save it as originalLanguage
        const languageMatch = translationNotes.match(/Language identified:\s*([^\n.]+)/i);
        if (languageMatch && languageMatch[1] && !extractedData.originalLanguage) {
          extractedData.originalLanguage = languageMatch[1].trim();
        }
      }
      
      console.log('Successfully extracted structured data from image response');
      return extractedData;
    } catch (error) {
      console.error(`Error processing image response for ${type}:`, error);
