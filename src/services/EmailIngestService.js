...
import { collection, addDoc, query, where, getDocs, getDoc, updateDoc, doc, serverTimestamp, orderBy, limit } from 'firebase/firestore';
...
class EmailIngestService {
  ...
  async getPersonalizedEmailAddress(familyId) {
    try {
      ...
      // Use EmailConfigurationService to get the family's email
      const email = await EmailConfigurationService.getFamilyEmail(familyId);
      
      if (!email) {
        throw new Error("Family email not configured");
      }
      ...
      return `family${familyId.substring(0, 8)}@families.checkallie.com`;
    }
  }
   ...
   */
  async getEmailHistory(familyId, maxResults = 10) {
    try {
      if (!familyId) {
        throw new Error("Family ID is required");
      }
