...
class QuestionFeedbackService {
  ...
  async recordQuestionFeedback(feedback) {
    try {
      ...
      // Create document ID using family, user and question ID for member-specific feedback
      const docId = feedback.userId 
        ? `${feedback.familyId}-${feedback.userId}-${feedback.questionId}`
        : `${feedback.familyId}-${feedback.questionId}`;
      ...
      const feedbackDoc = {
        ...
        userId: feedback.userId || null, // Store the user who provided the feedback
      ...
      // Update the member-specific list of excluded questions if not applicable
      if (feedback.feedbackType === 'not_applicable') {
        await this.updateMemberExcludedQuestions(feedback.familyId, feedback.userId, feedback.questionId, true);
      } else if (feedback.feedbackType === 'applicable') {
        // If marked as applicable, remove from excluded list if present
        await this.updateMemberExcludedQuestions(feedback.familyId, feedback.userId, feedback.questionId, false);
      }
  ...
  /**
   * Update the member-specific list of excluded questions
   * @param {string} familyId - Family ID
   * @param {string} userId - User ID (member ID)
   * @param {string} questionId - Question ID
   * @param {boolean} exclude - Whether to exclude (true) or include (false) the question
   * @returns {Promise<boolean>} Success indicator
   */
  async updateMemberExcludedQuestions(familyId, userId, questionId, exclude) {
    try {
      if (!userId) {
        // Fallback to family-wide exclusions if no userId provided
        return this.updateFamilyExcludedQuestions(familyId, questionId, exclude);
      }
      
      // Create a member-specific document ID
      const docRef = doc(db, "memberQuestionSettings", `${familyId}-${userId}`);
      ...
      console.error("Error updating member excluded questions:", error);
      return false;
    }
  }
  
  /**
   * Update the family's list of excluded questions (legacy support)
   * @param {string} familyId - Family ID
   * @param {string} questionId - Question ID
   * @param {boolean} exclude - Whether to exclude (true) or include (false) the question
   * @returns {Promise<boolean>} Success indicator
   ...
   * @param {string} familyId - Family ID
   * @param {string} memberId - Optional member ID for member-specific exclusions
  ...
  async getQuestionsToExclude(familyId, memberId = null) {
    try {
      ...
      // If we have a memberId, get member-specific exclusions
      if (memberId) {
        // Get member-specific exclusions
        const memberDocRef = doc(db, "memberQuestionSettings", `${familyId}-${memberId}`);
        const memberDocSnap = await getDoc(memberDocRef);
        
        if (memberDocSnap.exists()) {
          // Return only the member-specific exclusions
          return memberDocSnap.data().excludedQuestions || [];
        }
        
        // If no member-specific exclusions exist, return empty array
        // This ensures each member starts fresh with all questions
        return [];
      }
      
      // Legacy support: If no memberId provided, get family-wide exclusions
      const docRef = doc(db, "familyQuestionSettings", familyId);
      const docSnap = await getDoc(docRef);
      
      if (docSnap.exists()) {
        return docSnap.data().excludedQuestions || [];
      }
