...
/**
 * Backfill Palsson Family Data to Neo4j
 *
 * Cloud Function triggers only fire on NEW writes.
 * This script updates all existing Palsson family documents to trigger sync.
 *
 * Process:
 * 1. Find all Palsson family documents (tasks, events, chores)
...
async function backfillData() {
  console.log('\n🔄 Backfilling Palsson Family Data to Neo4j...\n');
  ...
  try {
    ...
    for (const doc of tasksSnapshot.docs) {
      ...
      if (count1 % 500 === 0) {
        ...
        batch1 = db.batch(); // Create new batch after commit
      }
    }
    if (count1 % 500 !== 0) {
      await batch1.commit();
    ...
    for (const doc of eventsSnapshot.docs) {
      ...
      if (count2 % 500 === 0) {
        ...
        batch2 = db.batch(); // Create new batch after commit
      }
    }
    if (count2 % 500 !== 0) {
      await batch2.commit();
    ...
    for (const doc of choresSnapshot.docs) {
      ...
      if (count3 % 500 === 0) {
        ...
        batch3 = db.batch(); // Create new batch after commit
      }
    }
    if (count3 % 500 !== 0) {
      await batch3.commit();
    ...
    for (const doc of fairPlaySnapshot.docs) {
      ...
      if (count4 % 500 === 0) {
        ...
        batch4 = db.batch(); // Create new batch after commit
      }
    }
    if (count4 % 500 !== 0) {
      await batch4.commit();
    ...
    try {
      const result = await session.run(`
        MATCH (n {familyId: 'palsson_family_simulation'})
      ...
      console.log('\n✅ SUCCESS! Palsson family data is now in Neo4j');
      console.log('\n🌐 Test at: https://checkallie.com');
      console.log('   Login: stefan@palssonfamily.com');
      console.log('   Go to: Balance & Habits → Take Weekly Survey\n');
      console.log('━'.repeat(60) + '\n');
